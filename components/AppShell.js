...
const ROUTES = [
  ...
  { href: '/favorites', label: 'Trusted people', icon: Users, featureFlag: 'favorites' },
...
export default function AppShell({ children }) {
  ...
  return <div className="min-h-screen md:grid md:grid-cols-[248px_1fr]">
    ...
    <div className="min-w-0">
      ...
      <nav aria-label="Primary mobile navigation" className="md:hidden fixed bottom-3 left-3 right-3 z-40 pb-[env(safe-area-inset-bottom)]">
        <div className="items-center rounded-[2rem] border border-white/10 bg-[#0b0414]/85 p-2 shadow-2xl shadow-black/50 backdrop-blur-2xl" style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.max(primaryNav.length, 1)}, minmax(0, 1fr))` }}>
          {primaryNav.map(item => {
...
function NavItem({ item, pathname, onClick, compact = false }) {
  const Active = pathname === item.href || pathname.startsWith(`${item.href}/`);
  const Icon = item.icon;
  return <Link href={item.href} onClick={onClick} className={`flex items-center justify-between gap-3 rounded-xl px-3 ${compact ? 'py-2 text-[13px]' : 'py-2.5 text-sm'} ${Active ? 'bg-gradient-to-r from-pink-500/18 to-purple-600/16 text-white border border-white/8' : 'text-white/65 hover:bg-white/5'}`}><span className="flex items-center gap-3"><Icon className="h-4 w-4" />{item.label}</span>{item.soon && <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] text-white/50">Soon</span>}</Link>;
}
