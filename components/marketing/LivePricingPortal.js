...
export default function LivePricingPortal() {
  ...
  useEffect(() => {
    setTarget(document.getElementById('pricing'));
    fetch('/api/plans')
      .then((response) => {
      ...
      .catch((requestError) => setError(requestError?.message || 'Plans are temporarily unavailable.'));
  }, []);
  ...
  return createPortal(
    <div data-snapnext-live-pricing className="mx-auto max-w-7xl">
      ...
      {error ? (
        <div className="mt-8 rounded-3xl border border-amber-400/25 bg-amber-400/10 p-6 text-center text-sm text-amber-100" role="status">{error}</div>
      ) : !plans.length ? (
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5" aria-label="Loading plans">
          {Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-[2rem] border border-white/10 bg-white/[0.03]" />)}
        </div>
      ) : (
