...
export default function MagicLibraryGallery() {
  ...
  return <div className="space-y-8">
    ...
    {magic.activePerson&&<button onClick={()=>magic.setActivePerson('')} className="rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-xs font-bold text-white/65">Show all memories</button>}{canAddMore&&<PeopleActivation people={magic.people} limit={magic.activation.limit} activeNames={magic.activation.active} draftNames={magic.draftNames} onToggle={magic.toggleDraft} onConfirm={confirm} busy={magic.activating}/>} 
    <div><h2 className="text-2xl font-black text-white">{title}</h2><p className="mt-1 text-sm text-white/45">{magic.visibleItems.length} matching protected memories</p></div><MediaSection title={magic.activePerson?`Best of ${displayName(magic.activePerson)} ✨`:'Best Matches ✨'} items={magic.bestItems} onOpen={setViewer}/><MediaSection title={magic.activePerson?`Videos with ${displayName(magic.activePerson)}`:'Videos'} items={magic.videos} onOpen={setViewer}/><MediaSection title="All Photos" items={magic.photos} onOpen={setViewer} emptyCopy="No matching photos yet."/><MediaViewer item={viewer} onClose={()=>setViewer(null)} onChanged={magic.reload}/><LockedPersonPrompt person={lockedPerson} onClose={()=>setLockedPerson(null)}/>
  </div>;
}
