...
export default function MagicLibraryGalleryMagic() {
  ...
  // Result links and the Add photos shortcut use a stable cluster ID. Respect it
  // only when the person is still active; arbitrary query parameters never
  // unlock a person or bypass the plan limit.
  useEffect(() => {
    if (typeof window === 'undefined' || !magic.people.length) return;
    const requested = String(new URLSearchParams(window.location.search).get('person') || '').trim();
    if (!requested || !magic.activation.enabled?.includes?.(requested) || magic.activePerson === requested) return;
    if (!magic.people.some((person) => (person.clusterId || person.name) === requested)) return;
    setSectionKey(null);
    setCategory(null);
    setDraftQuery('');
    magic.setQuery('');
    magic.setActivePerson(requested);
  }, [magic.people, magic.activation.enabled, magic.activePerson]);
    ...
    <div className="space-y-5">
      <header>
        <div className="flex items-center gap-2.5">
          <span className="grid h-9 w-9 place-items-center rounded-full bg-pink-500/15 text-pink-200"><Sparkles className="h-4 w-4" /></span>
          <div><h1 className="text-2xl font-black text-white md:text-3xl">Magic Library</h1><p className="text-xs text-white/40">Your organized memories</p></div>
        </div>
        <div className="mt-3 flex max-w-3xl gap-2">
          <div className="relative flex-1">
            ...
            {draftQuery && <button aria-label="Clear search" onClick={() => { setDraftQuery(''); magic.setQuery(''); }} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/35"><X className="h-4 w-4" /></button>}
          </div>
          <button onClick={() => runSearch()} disabled={!draftQuery.trim() || magic.searchBusy} className="inline-flex min-w-[92px] items-center justify-center gap-1.5 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">
            {magic.searchBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
            {magic.searchBusy ? 'Searching' : 'Search'}
          </button>
        </div>
