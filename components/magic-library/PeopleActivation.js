...
export default function PeopleActivation({ people, limit, activeNames, draftNames, onToggle, onConfirm, onSkip, busy }) {
  ...
  async function handleConfirm() {
    ...
    if (!canConfirm) {
      if (firstActivation && remaining > 0) {
        toast.error(`Choose ${remaining} more ${remaining === 1 ? 'person' : 'people'} to activate People Search.`);
      } else {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 md:p-7">
      ...
      {firstActivation && onSkip && <p className="mt-3 text-center text-xs text-white/35">You can activate People Search later from Favorites.</p>}
    </section>
  );
}
