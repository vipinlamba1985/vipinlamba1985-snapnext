      ...
      {orderedPeople.map((person) => {
        ...
        return <button key={person.name} onClick={() => setSelected({ person, enabled })} className="w-[6.25rem] shrink-0 snap-start text-center" aria-label={`${label}, ${person.count} memories`}>
          <span className={`relative mx-auto block h-24 w-24 overflow-hidden rounded-full border-[3px] bg-white/5 shadow-lg shadow-black/30 ${activePerson === person.name ? 'border-pink-400 ring-4 ring-pink-500/20' : person.verificationStatus === 'suggested' ? 'border-amber-300/70' : enabled ? 'border-white/25' : 'border-white/10'}`}>
            {person.representativeMediaId ? <img src={mediaSrc(person.representativeMediaId)} alt="" className={`h-full w-full object-cover ${enabled ? '' : 'opacity-75'}`} style={adjustedCrop(person.representativeFaceBox, profile)} /> : <span className="grid h-full w-full place-items-center font-black text-white/60">?</span>}
...
function PersonDialog({ person, enabled, limit, activeCount, profile, label, onSaveCrop, onClose, onOpen }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4 backdrop-blur" onClick={onClose}>
    <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-[#0b0414] p-5" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="flex items-center gap-4"><div className="h-28 w-28 shrink-0 overflow-hidden rounded-full border-4 border-pink-400/50 bg-white/5">{person.representativeMediaId && <img src={mediaSrc(person.representativeMediaId)} alt="" className="h-full w-full object-cover" style={adjustedCrop(person.representativeFaceBox, { x, y, zoom })} />}</div><div><h2 className="text-2xl font-black text-white">{name || 'Name this person'}</h2><p className="mt-1 text-sm text-white/50">{person.count || 0} related photos</p>{person.verificationStatus === 'suggested' && <p className="mt-2 text-xs font-bold text-amber-200">SnapNext is {Number(person.bestSimilarity || 0).toFixed(1)}% confident. Please confirm.</p>}</div></div>
      ...
      <div className="mt-4 space-y-3 rounded-2xl border border-white/10 bg-white/[0.035] p-4"><p className="flex items-center gap-2 text-sm font-black text-white"><Pencil className="h-4 w-4" />Center the face</p><Slider label="Left / right" value={x} setValue={setX} min={0} max={100} /><Slider label="Up / down" value={y} setValue={setY} min={0} max={100} /><Slider label="Zoom" value={zoom} setValue={setZoom} min={0.8} max={1.8} step={0.05} /></div>
