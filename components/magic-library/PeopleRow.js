...
import { Check, Heart, HelpCircle, LockKeyhole, Loader2, Move, Pencil, RotateCcw, Sparkles, UserCheck, UserX, X } from 'lucide-react';
...
function PersonDialog({ person, enabled, limit, activeCount, profile, label, onSaveCrop, onClose, onOpen }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/80 p-4 backdrop-blur" onClick={onClose}>
    <div className="relative my-4 w-full max-w-md rounded-3xl border border-white/10 bg-[#0b0414] p-5" onClick={(e) => e.stopPropagation()}>
      ...
      {unknown ? <button onClick={() => setIdentityState('person')} disabled={Boolean(busy)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-white/[0.08] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{busy === 'person' ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserCheck className="h-4 w-4" />}Restore as a person</button> : <>
        ...
        <button onClick={() => setIdentityState('unknown')} disabled={Boolean(busy)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-white/10 px-5 py-3 text-xs font-black text-white/55 disabled:opacity-50">{busy === 'unknown' ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserX className="h-4 w-4" />}Mark as Unknown</button>
        <p className="mt-2 text-center text-[10px] leading-4 text-white/30">Use this for strangers or faces you do not want in your active People row. It does not delete any photo.</p>
      </>}
    </div>
  </div>;
}
