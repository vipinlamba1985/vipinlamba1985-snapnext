...
export default function PeopleRow({ people, enabledNames, favoriteNames, activePerson, displayName, onEditProfile, onLocked }) {
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-white/40">Tap a face to edit the photo, name or private details.</p>
        </div>
      ...
      {!!active.length && <div><div className="mb-2 flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-pink-200/65"><Sparkles className="h-3 w-3" /> Active people</div><div className="flex gap-3 overflow-x-auto pb-1">{active.map((person) => <PersonTile key={person.name} person={person} enabled favoriteNames={favoriteNames} activePerson={activePerson} displayName={displayName} onEditProfile={onEditProfile} onLocked={onLocked} />)}</div></div>}

      {!!discovered.length && <div><div className="mb-2 flex items-center justify-between gap-3"><div className="text-[10px] font-black uppercase tracking-[0.14em] text-white/35">Discovered · {discovered.length}</div><div className="text-[10px] text-white/30">Visible on every plan</div></div><div className="flex gap-3 overflow-x-auto pb-1">{discovered.map((person) => <PersonTile key={person.name} person={person} enabled={false} favoriteNames={favoriteNames} activePerson={activePerson} displayName={displayName} onEditProfile={onEditProfile} onLocked={onLocked} />)}</div></div>}
    </section>
  );
}

function PersonTile({ person, enabled, favoriteNames, activePerson, displayName, onEditProfile, onLocked }) {
  ...
  return (
    <div className="w-16 shrink-0 text-center">
      <button onClick={() => enabled ? onEditProfile(person) : onLocked(person)} className="block w-full">
      ...
      <button onClick={() => enabled ? onEditProfile(person) : onLocked(person)} className="mt-1.5 inline-flex max-w-full items-center justify-center gap-1 text-[11px] font-bold text-white/70"><span className="truncate">{label}</span>{enabled && <Pencil className="h-2.5 w-2.5 text-white/30" />}</button>
