...
import { Check, Heart, HelpCircle, LockKeyhole, Loader2, Move, Pencil, RotateCcw, Search, Sparkles, UserCheck, Users, UserX, X } from 'lucide-react';
...
export default function PeopleRow({ people, enabledNames, activeCount, favoriteNames, activePerson, displayName, onOpen, onLocked }) {
  ...
  const featuredPeople = useMemo(() => orderedPeople.filter((person) => person.thumbnailEligible !== false || person.isSelf || enabledNames.includes(person.name)), [orderedPeople, enabledNames]);
  const displayedPeople = showAllFaces ? orderedPeople : featuredPeople;
  const hiddenFaceCount = Math.max(0, orderedPeople.length - featuredPeople.length);
  ...
  function openMemories(person, enabled) {
    ...
    if (person.thumbnailEligible === false) onOpen(person.name);
    else if (enabled) onOpen(person.name);
    else onLocked?.(person);
  }

  return <section className="space-y-3">
    <div className="flex items-center justify-between gap-3">
      <div><h2 className="text-xl font-black text-white">People</h2><p className="text-xs leading-5 text-white/40">A People card appears after the same face is found in 3 different photos. Tap a photo to edit it; tap the name to view memories.</p></div>
    ...
    {displayedPeople.length > 0 ? <div className="flex snap-x gap-4 overflow-x-auto pb-3 pr-8 [mask-image:linear-gradient(to_right,black_94%,transparent)]">
      {displayedPeople.map((person) => {
        const unknown = isUnknownPerson(person);
        const enabled = !unknown && enabledNames.includes(person.name);
        const lowFrequency = !unknown && person.thumbnailEligible === false;
        const profile = cropFor(person);
        const label = cleanLabel(person, displayName);
        const memoryCount = Number(person.count || 0);
        const photoCount = Number(person.distinctPhotoCount ?? person.photos ?? memoryCount);
        return <div key={person.name} className={`w-[7.25rem] shrink-0 snap-start text-center ${unknown ? 'opacity-60' : ''}`}>
          ...
          <button type="button" disabled={unknown} onClick={() => openMemories(person, enabled)} className={`mt-2 flex min-h-14 w-full flex-col items-center justify-center rounded-2xl border px-2 py-2 text-center transition focus:outline-none focus-visible:ring-4 focus-visible:ring-pink-500/30 ${unknown ? 'cursor-default border-white/5 bg-white/[0.025]' : lowFrequency ? 'border-sky-300/20 bg-sky-300/[0.07] active:scale-[0.98]' : enabled ? 'border-pink-400/25 bg-pink-500/10 active:scale-[0.98]' : 'border-white/10 bg-white/[0.055] active:scale-[0.98]'}`} aria-label={unknown ? `Unknown face, ${memoryCount} memories` : `${label}, open ${memoryCount} memories`}>
            <span className="block w-full truncate text-sm font-black text-white/90">{label}</span>
            <span className={`mt-0.5 block text-[11px] font-bold ${enabled ? 'text-pink-200' : lowFrequency ? 'text-sky-200' : 'text-white/45'}`}>{unknown ? `${memoryCount} memories` : lowFrequency ? `Detected in ${photoCount} ${photoWord(photoCount)} · View` : enabled ? `View ${memoryCount} memories` : `Activate to view · ${memoryCount}`}</span>
          </button>
        </div>;
      })}
    </div> : <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-sm text-white/55">No recurring face has reached 3 different photos yet. Your one-off detected faces remain available under Show all faces.</div>}
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-2 text-[10px] text-white/35"><Sparkles className="h-3 w-3 text-pink-200/70" />Large group photos stay in Group photos and are kept out of Best unless another active person is present.</div>
      <div className="flex flex-col gap-2 sm:flex-row">
        {(hiddenFaceCount > 0 || showAllFaces) && <button type="button" onClick={() => setShowAllFaces((value) => !value)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-sky-300/20 bg-sky-300/[0.07] px-4 py-2 text-xs font-black text-sky-100 active:scale-[0.98]"><Users className="h-4 w-4" />{showAllFaces ? 'Show recurring people' : `Show all faces · ${orderedPeople.length}`}</button>}
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/80 p-4 backdrop-blur" onClick={onClose}>
    <div className="relative my-4 w-full max-w-md rounded-3xl border border-white/10 bg-[#0b0414] p-5" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="text-center">
        <div className={`mx-auto w-fit rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-wider ${person.isSelf ? 'border-emerald-300/25 bg-emerald-400/10 text-emerald-200' : unknown ? 'border-white/10 bg-white/5 text-white/55' : lowFrequency ? 'border-sky-300/25 bg-sky-300/10 text-sky-100' : 'border-emerald-300/20 bg-emerald-400/10 text-emerald-200'}`}>{person.isSelf ? 'Your face' : unknown ? 'Unknown face' : lowFrequency ? `${photoCount} of 3 photos` : 'Automatic face focus'}</div>
        <PeopleFaceThumbnail mediaId={person.representativeMediaId} faceBox={person.representativeFaceBox} manual={crop} editable={!unknown} onManualChange={updateCrop} className={`mx-auto mt-4 h-64 w-44 rounded-[2rem] border-4 shadow-2xl shadow-black/50 ${unknown ? 'border-white/15 grayscale' : lowFrequency ? 'border-sky-300/45' : 'border-pink-400/50'}`} />
        ...
        {unknown ? <p className="mt-2 text-xs leading-5 text-white/40">This face stays available under Show all faces but does not use an active slot. The original photos remain in your library.</p> : lowFrequency ? <p className="mt-2 text-xs leading-5 text-sky-100/70">SnapNext has found this face in {photoCount} different {photoWord(photoCount)}. It will automatically join the main People row after 3 matching photos.</p> : person.verificationStatus === 'suggested' && <p className="mt-2 text-xs font-bold text-amber-200">SnapNext is {Number(person.bestSimilarity || 0).toFixed(1)}% confident. Please confirm.</p>}
      </div>
      {unknown ? <div className="mt-5 space-y-2">
        ...
        <p className="mt-2 text-center text-[10px] leading-4 text-white/30">Use Unknown for strangers or faces you do not want in People. It does not delete any photo.</p>
      </>}
    </div>
  </div>;
}
