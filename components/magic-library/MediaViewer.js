...
export default function MediaViewer({ item, items = [], index = 0, onClose, onChanged }) {
  ...
  async function download() {
    ...
    a.href = url;
    a.download = current.name || 'memory';
    a.click();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur" onClick={onClose}>
      ...
      <div className="flex h-full flex-col" onClick={(event) => event.stopPropagation()}>
        <div className="flex-1 grid place-items-center p-4 pt-16 pb-[22rem] md:pb-[19rem]">
        ...
        <div className="absolute bottom-0 left-0 right-0 max-h-[48vh] overflow-y-auto border-t border-white/10 bg-[#0b0414]/95 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          ...
          <div className="mb-3 grid gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3 md:grid-cols-2">
            <label className="block">
              ...
              {selectedCategory === 'screenshots' && <p className="mt-1.5 text-[11px] text-white/40">Use this menu to move this screenshot into Docs.</p>}
            </label>
