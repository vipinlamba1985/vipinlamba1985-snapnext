      ...
      {items.length === 0 ? <p className="mt-3 text-sm text-white/40">{emptyCopy || 'Nothing found here yet.'}</p> : (
        <div className="mt-4 flex gap-3.5 overflow-x-auto pb-3 pr-5">
          {items.slice(0, 14).map((item, index) => (
            <button key={item.id} onClick={() => onOpen(item, index)} className="group relative h-32 w-32 shrink-0 overflow-hidden rounded-2xl bg-white/5 text-left md:h-40 md:w-40">
