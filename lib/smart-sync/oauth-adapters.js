const ADAPTERS = {
  ...
  google_photos: {
    ...
    callbackPath: '/api/smart-sync/oauth/google_photos/callback',
  ...
  dropbox: {
    ...
    callbackPath: '/api/smart-sync/oauth/dropbox/callback',
    clientIdEnv: 'DROPBOX_CLIENT_ID',
    clientSecretEnv: 'DROPBOX_CLIENT_SECRET',
  },
  onedrive: {
    scopes: ['offline_access', 'Files.Read'],
    callbackPath: '/api/smart-sync/oauth/onedrive/callback',
...
function oneDriveTenant() {
  const value = String(process.env.ONEDRIVE_TENANT_ID || 'common').trim();
  return /^[a-zA-Z0-9._-]+$/.test(value) ? value : 'common';
}

export function oauthAdapter(providerId) {
  const adapter = ADAPTERS[providerId];
  if (!adapter) return null;
  if (providerId !== 'onedrive') return adapter;
  const tenant = oneDriveTenant();
  return {
    ...adapter,
