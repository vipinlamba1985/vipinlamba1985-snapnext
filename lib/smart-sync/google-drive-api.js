...
import { inspectDriveGrant } from '@/lib/google-drive-scope';
...
export const DRIVE_FIELDS = 'id,name,mimeType,size,createdTime,modifiedTime,thumbnailLink,starred,md5Checksum,sha1Checksum,sha256Checksum,version,trashed';
const MAX_DISCOVERY_ITEMS = 500;

/**
 * Thrown when a connection still carries a grant wider than `drive.file`.
 * Callers should stop rather than continue with reduced expectations — the
 * point is that the credential must not be used at all.
 */
export class DriveRescopeRequiredError extends Error {
  constructor(reason) {
    super('Google Drive permissions were narrowed to the photos you choose. Please reconnect.');
    this.name = 'DriveRescopeRequiredError';
    this.code = 'rescope_required';
    this.reason = reason;
  }
}

/**
 * Every Drive credential in the application passes through here, which is why
 * the scope guard lives at this level rather than only on the routes a person
 * visits.
 *
 * Guarding the user-facing endpoints alone left a real gap: the background sync
 * worker refreshes tokens on a schedule, so a connection still holding
 * `drive.readonly` could have been refreshed and used by cron long before
 * anyone opened a page that would have revoked it. A credential that should not
 * exist must fail closed wherever it is picked up.
 */
export async function freshGoogleDriveAccessToken(db, connection) {
  const grant = inspectDriveGrant(connection?.grantedScope);
  if (!grant.ok) throw new DriveRescopeRequiredError(grant.reason);
