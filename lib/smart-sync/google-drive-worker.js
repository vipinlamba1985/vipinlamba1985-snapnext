...
export async function processGoogleDriveJobBatch({ db, jobId, userId = null }) {
  ...
  try {
    ...
    if (!Array.isArray(job.sourceFileIds) || !job.sourceFileIds.length) {
      ...
      const estimatedItems = Math.max(Number(job.processedItems) || 0, Number(job.estimatedItems) || 0) + sourceFileIds.length;
      const estimatedBytes = (Number(job.estimatedBytes) || 0) + candidates.reduce((sum, item) => sum + Math.max(0, Number(item.size || 0)), 0);
      ...
      await db.collection('smart_sync_jobs').updateOne(
        { id: job.id, userId: job.userId, leaseToken: job.leaseToken },
        { $set: { sourceFileIds, estimatedItems, estimatedBytes, sourceCursorAt, sourceOldestAt, initialMode, cursorIndex: 0, updatedAt: new Date() } },
      );
      job = { ...job, sourceFileIds, estimatedItems, estimatedBytes, sourceCursorAt, sourceOldestAt, initialMode, cursorIndex: 0 };
    }
    ...
    const initialChunkFinished = sourceBatchFinished && job.initialMode;
    const finished = sourceBatchFinished && !job.initialMode;
    const update = {
      cursorIndex: initialChunkFinished ? 0 : nextCursor,
      sourceFileIds: initialChunkFinished ? [] : job.sourceFileIds,
    ...
    if (sourceBatchFinished && maxModifiedAt.getTime()) {
      const set = { lastAutoSyncAt: new Date(), updatedAt: new Date() };
      if (job.initialMode) {
    ...
    return { claimed: true, completed: finished, initialChunkFinished, capacityReached, totals };
  } catch (error) {
    await releaseLease(
      db,
      job,
