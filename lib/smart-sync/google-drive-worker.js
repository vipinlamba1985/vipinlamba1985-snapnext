...
async function listDriveCandidates(token, { since, rules, limit = MAX_SOURCE_FILES } = {}) {
  const items = [];
  let pageToken = '';
  while (items.length < limit) {
    const queryParts = ["trashed = false", "(mimeType contains 'image/' or mimeType contains 'video/')"];
    if (since) queryParts.push(`modifiedTime > '${new Date(since).toISOString()}'`);
...
export async function processGoogleDriveJobBatch({ db, jobId, userId = null }) {
  ...
  try {
    ...
    if (!Array.isArray(job.sourceFileIds) || !job.sourceFileIds.length) {
      const since = connection.smartSyncInitialCompleted ? connection.smartSyncCursorAt || connection.connectedAt : null;
      const candidates = await listDriveCandidates(token, { since, rules: enabledRules(job) });
      if (!candidates.length) {
        await releaseLease(db, job, terminalJobPatch('completed', { completionReason: 'no_changes' }), { activeKey: '' });
        await db.collection('cloud_connections').updateOne(
          { _id: connection._id },
          { $set: { lastAutoSyncAt: new Date(), smartSyncInitialCompleted: true, updatedAt: new Date() } },
        );
      ...
      await db.collection('smart_sync_jobs').updateOne(
        { id: job.id, userId: job.userId, leaseToken: job.leaseToken },
        { $set: { sourceFileIds, estimatedItems: sourceFileIds.length, estimatedBytes, sourceCursorAt, updatedAt: new Date() } },
      );
      job = { ...job, sourceFileIds, estimatedItems: sourceFileIds.length, estimatedBytes, sourceCursorAt };
    }
    ...
    const update = {
      cursorIndex: capacityReached ? job.sourceFileIds.length : nextCursor,
      processedItems: (Number(job.processedItems) || 0) + totals.processed,
      importedItems: (Number(job.importedItems) || 0) + totals.saved,
      skippedItems: (Number(job.skippedItems) || 0) + totals.skipped + (capacityReached ? job.sourceFileIds.length - nextCursor : 0),
    ...
    if (finished && maxModifiedAt.getTime()) {
      await db.collection('cloud_connections').updateOne(
        { _id: connection._id },
        { $set: { smartSyncCursorAt: maxModifiedAt, smartSyncInitialCompleted: true, lastAutoSyncAt: new Date(), updatedAt: new Date() } },
      );
    }
