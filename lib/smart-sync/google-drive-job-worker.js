...
import { normalizeSmartSyncMode, selectDriveProtection } from '@/lib/smart-sync/selection';
...
function ruleRank(meta, rules) {
  ...
  const index = rules.findIndex(rule => {
    if (rule.type === 'favorites') return Boolean(meta.starred);
...
function prepareDiscoveryResult({ job, items, inventory, initialMode, nextPageToken, newStartPageToken, removedItems = 0, metrics }) {
  ...
  return {
    ...
    automaticallySkipped: inventory.safeExisting + inventory.unsupported + removedItems,
    nextPageToken,
    newStartPageToken,
    metrics,
  };
...
function noImportCompletionReason(job, patch, discovery) {
  const syncMode = normalizeSmartSyncMode(job.syncMode);
  if (syncMode === 'index_only' && (patch.indexedItems > 0 || discovery.metrics.discoveredItems > 0)) return 'indexed';
  if (syncMode === 'protect_important' && patch.indexedItems > 0) {
    return Number(job.importedItems || 0) > 0 ? 'protected_important' : 'no_priority_matches';
  }
  return 'no_changes';
}
...
async function completeDiscoveryWithoutImports({ db, job, connection, discovery }) {
  ...
  if (discovery.nextPageToken) {
    ...
    await updateConnectionMetrics(db, connection, discovery.metrics, {
  ...
  const completionReason = noImportCompletionReason(job, patch, discovery);
  await releaseLease(
    db,
    job,
  ...
  return { claimed: true, completed: true, reason: completionReason };
}

function finishedCompletionReason(job) {
  const syncMode = normalizeSmartSyncMode(job.syncMode);
  if (syncMode === 'protect_important') return 'protected_important';
  if (syncMode === 'index_only') return 'indexed';
  return 'finished';
}

/**
 * Legacy whole-Drive sync jobs cannot run under the per-file scope: drive.file
 * has no ability to enumerate a Drive, so any queued row predates the migration
 * and would only retry against credentials that are deliberately unusable.
 *
 * Removing the scheduled cron stopped new work being created, but a manual
 * trigger or a generic runner could still claim an old row. This refuses them
 * at the entry point until the durable runner treats DriveRescopeRequiredError
 * as terminal on its own.
 */
export const LEGACY_DRIVE_SYNC_DISABLED = true;

export async function processGoogleDriveJobBatch({ db, jobId, userId = null }) {
  if (LEGACY_DRIVE_SYNC_DISABLED) {
    await db.collection('smart_sync_jobs').updateOne(
      { id: jobId },
      { $set: { status: 'cancelled', cancelledReason: 'drive_sync_retired', updatedAt: new Date() } },
    ).catch(() => {});
    return { processed: 0, cancelled: true, reason: 'drive_sync_retired' };
  }
  ...
  try {
    ...
    const update = {
      ...
      completionReason: capacityReached ? 'capacity_reached' : finished ? finishedCompletionReason(job) : null,
