...
export function normalizeDriveAsset(meta = {}) {
  ...
  return {
    ...
    favorite: Boolean(meta.starred || meta.favorite),
...
export async function markCloudAsset({
  ...
  // Cloud provider importers converge here after bytes have been copied and a
  // media row exists. Mark only genuinely new versions, never inventory scans
  // or duplicate skips, so cloud imports participate in the same debounced
  // Magic regeneration contract without widening every provider implementation.
  const createdMedia = patch?.mediaId
    && ['copied_and_verified', 'new_provider_version'].includes(String(patch?.importOutcome || ''));
  if (createdMedia) {
    await db.collection('media').updateOne(
      { userId, id: String(patch.mediaId) },
      { $set: { ...pendingMagicEligibilityFields(now), uploadedAt: patch.importedAt || now } },
    );
    await markMagicManifestDirty(db, userId, 'asset_added').catch((error) => {
      // Import bytes and metadata are already safely committed. Secondary Magic
      // invalidation must not turn that success into a provider retry/failure.
      console.error('[cloud-import] could not mark Magic manifest dirty:', error?.message || error);
    });
  }
}
