...
async function cleanupPickerSession({ db, token, job }) {
  if (job.providerId !== 'google_photos' || !job.pickerSessionId) return;
  try {
    await deleteGooglePhotosPickerSession(token, job.pickerSessionId);
  } catch {}
  await db.collection('smart_sync_picker_sessions').deleteOne({
    userId: job.userId,
    provider: 'google_photos',
    sessionId: job.pickerSessionId,
  }).catch(() => {});
}
...
async function completePickerIndexOnly({ db, token, job, connection }) {
...
export async function processProviderJobBatch({ db, jobId, userId = null }) {
  ...
  try {
    ...
    if ((!Array.isArray(job.sourceFileIds) || !job.sourceFileIds.length) && provider === 'google_photos') {
      return completePickerIndexOnly({ db, token, job, connection });
    }
    ...
    if (finished && provider === 'google_photos') await cleanupPickerSession({ db, token, job });
