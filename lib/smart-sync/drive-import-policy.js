...
/**
 * Per-file ceiling for Drive imports.
 *
 * Deliberately far below what the ordinary uploader accepts. The download path
 * still reads the whole file into memory before validating it, so a large video
 * would exhaust a serverless function — and repeated large imports would be a
 * denial-of-service against ourselves, not merely a robustness problem.
 *
 * This cap is a stand-in for the streaming multipart pipeline, not a product
 * decision. When downloads stream to storage and validate as they go, raise it
 * to match the uploader and delete this note.
 */
export const MAX_IMPORT_BYTES = 150 * 1024 ** 2;

/** What the uploader accepts, for the message shown when Drive cannot match it. */
...
export function rejectionMessage(reason) {
  return {
    ...
    [REJECTION.TOO_LARGE]: 'This file is too large to import from Google Drive yet. Upload it directly from your device instead.',
