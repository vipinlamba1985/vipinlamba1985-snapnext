...
import { listPublicProviderStatus } from './providers.js';
...
function hasIncrementalCursor(connection) {
  if (!connection) return false;
  if (connection.provider === 'google_drive') return Boolean(connection.driveChangePageToken);
  if (connection.provider === 'dropbox') return Boolean(connection.dropboxCursor);
  if (connection.provider === 'onedrive') return Boolean(connection.oneDriveDeltaLink);
  return false;
}
...
export async function getSmartSyncState({ db, userId }) {
  ...
  const readiness = new Map(listPublicProviderStatus().map(provider => [provider.id, provider]));
  ...
  return {
    profile,
    // `readiness` already holds the public shape for every provider: all the
    // registry fields, `available` derived from configured credentials, and no
    // env var names. Only connection state is per-user, so that is all we add.
    // Spreading the raw registry entry here would put `env` back on the wire.
    ...
    operations: selectedConnection ? {
      ...
      syncMode: selectedConnection.smartSyncMode || profile.syncMode,
      initialDiscoveryComplete: Boolean(selectedConnection.smartSyncInitialCompleted),
      incrementalCursorReady: hasIncrementalCursor(selectedConnection),
...
export async function saveSmartSyncProfile({ db, userId, body = {} }) {
  ...
  if (provider?.surface === 'web') {
    await db.collection('cloud_connections').updateOne(
      ...
      {
        $set: {
          autoSyncEnabled: profile.enabled && provider.syncStrategy === 'durable_cloud_job',
          smartSyncMode: profile.syncMode,
