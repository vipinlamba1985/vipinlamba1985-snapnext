...
const STRONG_PROVIDER_HASHES = new Set(['sha256', 'sha1', 'dropbox_content_hash']);
...
export function isStrongProviderChecksum(checksum = {}) {
  return Boolean(checksum?.value && STRONG_PROVIDER_HASHES.has(String(checksum.algorithm || '').toLowerCase()));
}
...
export async function importCloudProviderAsset({ db, provider, token, user, providerFileId, usedBytes, jobId = null }) {
  ...
  if (isStrongProviderChecksum(asset.providerChecksum) && declaredSize) {
