...
async function associateFacesChecked(input) {
  const result = await send('AssociateFaces', input);
  if (Array.isArray(result?.UnsuccessfulFaceAssociations) && result.UnsuccessfulFaceAssociations.length > 0) {
    const error = new Error('AWS could not verify the Favourite Person reference face association.');
    error.code = 'favorite_reference_association_failed';
    error.unsuccessfulFaceAssociations = result.UnsuccessfulFaceAssociations.length;
    throw error;
  }
  return result;
}
...
export const peopleRekognition = {
  ...
  associateFaces: (input) => associateFacesChecked(input),
