...
async function recordLedgerEntry(db, entry) {
  ...
  const subtype = String(entry.subtype || 'revenue');
  const sourceObjectId = String(entry.sourceObjectId || sourceEventId || '');
  if (!sourceEventId || !sourceObjectId) throw new Error('Stripe ledger idempotency identifiers are required.');

  const ledgerKey = `${subtype}:${sourceObjectId}`;
  await db.collection('financial_ledger').updateOne(
    { source: 'stripe', ledgerKey },
    {
      $setOnInsert: {
        ...
        source: 'stripe',
        ledgerKey,
        firstSourceEventId: sourceEventId,
        createdAt: new Date(),
      },
      $set: {
        ...entry,
