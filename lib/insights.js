...
export async function computeInsights(db, user) {
  ...
  const remaining = isSuper(user) ? Number.MAX_SAFE_INTEGER : Math.max(0, plan.storageBytes - totalBytes);
  const monthsLeft = monthlyAvg > 0 && !isSuper(user) ? Math.floor(remaining / monthlyAvg) : null;
  ...
  // --- usage % ---
  const usagePct = isSuper(user) ? 0 : Math.min(100, Math.round((totalBytes / plan.storageBytes) * 100));

  return {
    totals: { count: totalCount, bytes: totalBytes },
    plan: { id: plan.id, name: plan.name, storageBytes: plan.storageBytes, isSuper: isSuper(user) },
    mostPhotographed,
    mostActive,
