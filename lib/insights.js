...
export async function computeInsights(db, user, request) {
  ...
  const remaining = isSuperUser(user, request) ? Number.MAX_SAFE_INTEGER : Math.max(0, plan.storageBytes - totalBytes);
  ...
  // --- usage % ---
  const usagePct = isSuperUser(user, request) ? 0 : Math.min(100, Math.round((totalBytes / plan.storageBytes) * 100));

  return {
    totals: { count: totalCount, bytes: totalBytes },
    plan: { id: plan.id, name: plan.name, storageBytes: plan.storageBytes, isSuper: isSuperUser(user, request) },
    mostPhotographed,
    mostActive,
