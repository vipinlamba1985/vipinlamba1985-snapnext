...
import { effectivePlan, isSuperUser } from '@/lib/entitlements';
import { listAcceptedFavoriteUserIds } from '@/lib/favorites';

const MONTH = ['January','February','March','April','May','June','July','August','September','October','November','December'];

export async function computeInsights(db, user) {
  ...
  const remaining = isSuperUser(user) ? Number.MAX_SAFE_INTEGER : Math.max(0, plan.storageBytes - totalBytes);
  const monthsLeft = monthlyAvg > 0 && !isSuperUser(user) ? Math.floor(remaining / monthlyAvg) : null;
  ...
  // --- usage % ---
  const usagePct = isSuperUser(user) ? 0 : Math.min(100, Math.round((totalBytes / plan.storageBytes) * 100));

  return {
    totals: { count: totalCount, bytes: totalBytes },
    plan: { id: plan.id, name: plan.name, storageBytes: plan.storageBytes, isSuper: isSuperUser(user) },
    mostPhotographed,
    mostActive,
