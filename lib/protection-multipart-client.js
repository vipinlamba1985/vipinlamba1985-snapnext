...
async function uploadPart(url, blob, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) resolve(xhr.getResponseHeader('ETag') || null);
...
export async function uploadProtectedMultipart(item, decision, onProgress) {
  const status = await apiFetch('/protection/multipart/status', {
    method: 'POST',
    body: JSON.stringify({ reservationId: decision.reservationId }),
  });
  const partSize = Number(status.partSize || decision.partSize || 16 * 1024 * 1024);
  const totalParts = Number(status.totalParts || decision.totalParts || Math.ceil(item.size / partSize));
  const stored = await loadMultipartResume(decision.reservationId);
  const completed = new Map();
  for (const part of stored?.parts || []) completed.set(part.partNumber, part);
  for (const part of status.parts || []) completed.set(part.partNumber, part);
