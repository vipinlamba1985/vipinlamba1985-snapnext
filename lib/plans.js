...
//
// Product strategy:
//  - Free proves the core memory value with a small, protected external AI allowance.
...
export const PLANS = {
  free: {
    ...
    features: ['15 GB storage', '20 AI Credits each week', 'Local & internal AI organization', 'Memory search', 'Basic gallery & memories'],
  ...
  plus: {
    ...
    features: ['100 GB storage', '80 AI Credits each week', 'AI Studio', 'Favorite sharing', 'Larger downloads'],
    aiFeatures: { chat: true, studio: true, video: false, command: false },
    color: 'from-fuchsia-500 to-purple-600', popular: false,
    prices: { monthly: { amount: 4.99, stripePriceId: process.env.STRIPE_PRICE_PLUS_MONTHLY || null }, yearly: { amount: 49.99, stripePriceId: process.env.STRIPE_PRICE_PLUS_YEARLY || null } },
  },
  pro: {
    ...
    features: ['1 TB storage', '180 AI Credits each week', 'AI Studio + AI Video', 'AI Command', 'Priority export'],
    aiFeatures: { chat: true, studio: true, video: true, command: true },
    color: 'from-pink-500 via-fuchsia-500 to-indigo-600', popular: true,
    prices: { monthly: { amount: 9.99, stripePriceId: process.env.STRIPE_PRICE_PRO_MONTHLY || null }, yearly: { amount: 99.99, stripePriceId: process.env.STRIPE_PRICE_PRO_YEARLY || null } },
  },
  family: {
    ...
    features: ['2 TB family vault', '280 shared AI Credits each week', 'Full SnapNext AI suite', 'Family & Favorite sharing', 'People sync & shared albums'],
    aiFeatures: { chat: true, studio: true, video: true, command: true },
...
export function hasAnyStripePrices() { return ['plus', 'pro', 'family'].some(id => !!PLANS[id].prices.monthly.stripePriceId || !!PLANS[id].prices.yearly.stripePriceId); }
