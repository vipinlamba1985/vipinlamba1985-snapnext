...
async function currentConfig(db) {
  ...
  return {
    ...
    // Keep the legacy field for admin/backward compatibility. The effective
    // ceiling is now shared by external AI and other metered product work such
    // as canonical Reel rendering.
...
async function productSpendForMonth(db, start, end) {
  const settledRows = await db.collection('product_cost_ledger').aggregate([
    {
      $match: {
        status: 'settled',
        createdAt: { $gte: start, $lt: end },
  ...
  const activeReservations = await db.collection('product_cost_reservations').aggregate([
    {
      $match: {
        status: 'reserved',
        expiresAt: { $gt: new Date() },
...
export async function getAiProfitGuardSnapshot({ db, now = new Date() }) {
  ...
  const [revenue, config, aiSpend, productSpend] = await Promise.all([
    recognizedRevenueForMonth(db, start, end),
    currentConfig(db),
    aiSpendForMonth(db, start, end),
    productSpendForMonth(db, start, end),
  ]);
  ...
  const committedAiSpend = aiSpend.settled + aiSpend.reserved;
  const committedProductSpend = productSpend.settled + productSpend.reserved;
  ...
  return {
    ...
    // Backward-compatible aliases remain so existing AI dashboards and gates do
    // not break while the underlying authority now covers total metered work.
    ...
    settledAiSpendUsd: Number(aiSpend.settled.toFixed(6)),
    reservedAiSpendUsd: Number(aiSpend.reserved.toFixed(6)),
    settledProductSpendUsd: Number(productSpend.settled.toFixed(6)),
    reservedProductSpendUsd: Number(productSpend.reserved.toFixed(6)),
...
export async function reserveAiSpend({ db, feature, agentId = null, userId = null, estimatedCostUsd, essential = false, metadata = {} }) {
  ...
  if (estimate > snapshot.remainingExternalWorkBudgetUsd) {
