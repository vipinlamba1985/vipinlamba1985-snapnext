...
export const CREATIVE_FEATURES = Object.freeze({
  ...
  // Smart search embeds descriptions the enrichment pipeline already wrote, so
  // it never re-analyses an image. That makes it far cheaper than the other
  // metered features — but it is still an external model call, so it is metered
  // rather than declared free.
  smart_search_index: {
    id: 'smart_search_index',
    label: 'Smart search indexing',
    billing: CREATIVE_BILLING.METERED,
    // Ceiling for one batch. Embedding text is priced per thousand tokens, so
    // the gateway releases almost all of this back.
    credits: 1,
    grounding: 'Reads the descriptions already saved with your photos. Your images are never sent again.',
  },
  smart_search_query: {
    id: 'smart_search_query',
    label: 'Smart search',
    billing: CREATIVE_BILLING.METERED,
    credits: 1,
    grounding: 'Turns what you typed into a search. Nothing about your photos leaves SnapNext.',
  },
});
