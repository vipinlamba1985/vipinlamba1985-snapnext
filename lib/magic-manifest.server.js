...
async function acquireUserLease(db, state, now, config) {
  ...
  return result.matchedCount === 1 ? { owner, leaseUntil } : null;
}
...
async function processBackfillUsers({ db, now, config, deadline, processedUserIds, remaining }) {
  ...
  const ids = await nextBackfillUserIds(db, state?.cursor || '', Math.max(remaining * 3, remaining));
  if (!ids.length) {
  ...
  for (const userId of ids) {
    ...
    if (dirtyState?.dirty) continue;

    const current = await db.collection('magic_manifest').findOne(
      { user_id: userId },
      { projection: { blueprint_version: 1 } },
    );
    if (current?.blueprint_version === MAGIC_BLUEPRINT_VERSION) continue;
  ...
  await backfills.updateOne(
    { _id: MAGIC_BLUEPRINT_VERSION },
    {
      $set: {
        cursor: lastCursor,
        completed: ids.length < Math.max(remaining * 3, remaining),
        updated_at: new Date(),
      },
