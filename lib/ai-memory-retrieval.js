...
/**
 * How many stored vectors one semantic search may scan.
 *
 * Similarity is computed in the application rather than by a database vector
 * index, so this is a real ceiling: libraries beyond it are searched
 * semantically only across their most recent photos, though keyword search
 * still covers everything. Moving to a database vector index removes the cap —
