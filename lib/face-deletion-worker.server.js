...
import {
  clearFaceDeletionRetryNotification,
  publishFaceDeletionRetryNotification,
} from '@/lib/face-deletion-notifications.server';
...
async function deleteRekognitionCollection(collectionId) {
  try {
    ...
    return { collectionId, alreadyAbsent: false };
  } catch (error) {
    if (isCollectionMissing(error)) return { collectionId, alreadyAbsent: true };
    throw error;
  }
}
...
async function verifyRekognitionCollectionAbsent(collectionId) {
  try {
    ...
    return { collectionId, ok: false, reason: 'collection_still_exists' };
  } catch (error) {
    if (isCollectionMissing(error)) return { collectionId, ok: true };
    throw error;
  }
}

async function deleteRekognitionCollections(collectionIds) {
  const results = [];
  for (const collectionId of [...new Set(collectionIds.filter(Boolean))]) {
    results.push(await deleteRekognitionCollection(collectionId));
  }
  return results;
}

async function verifyRekognitionCollectionsAbsent(collectionIds) {
  const collections = [];
  for (const collectionId of [...new Set(collectionIds.filter(Boolean))]) {
    collections.push(await verifyRekognitionCollectionAbsent(collectionId));
  }
  return { ok: collections.every((row) => row.ok), collections };
}
...
export async function processFaceDeletionGeneration({ db, userId, generation, workerId = crypto.randomUUID() }) {
  ...
  const collectionIds = [peopleCollectionId(userId), favoritePeopleCollectionId(userId)];
  try {
    const cloudDeletion = await deleteRekognitionCollections(collectionIds);
    await ownedUpdate(db, {
      userId, generation, workerId,
      filter: { status: 'processing' },
      set: { stage: 'cloud_deleted', cloudDeletion, leaseExpiresAt: new Date(Date.now() + config.leaseMs) },
    });
    ...
    const [awsVerification, snapNextVerification] = await Promise.all([
      verifyRekognitionCollectionsAbsent(collectionIds),
    ...
    await clearFaceDeletionRetryNotification({ db, userId, generation }).catch((error) => {
      console.warn('[face-deletion] could not clear retry notification', error?.message);
    });
    ...
    await publishFaceDeletionRetryNotification({ db, userId, generation }).catch((notifyError) => {
      console.warn('[face-deletion] could not publish retry notification', notifyError?.message);
    });
