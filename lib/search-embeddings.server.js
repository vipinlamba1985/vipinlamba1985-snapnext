...
/**
 * Cache key for a search phrase.
 *
 * Only the hash is stored, never the phrase. The vector for "beach" is
 * identical for everyone, so the cache is shared across users and each distinct
 * phrase is paid for once ever — but what any individual searched for is not
 * recoverable from the cache.
 */
function queryCacheKey(text) {
  return crypto
    .createHash('sha256')
    .update(`${EMBEDDING_VERSION}:${EMBEDDING_MODEL}:${text.toLowerCase()}`)
    .digest('hex');
}

/**
 * Embeds one search query, reusing a previous embedding of the same phrase.
 *
 * Without this, searching "beach" twice would be charged twice, and a user
 * paging or re-running a search would pay each time. Separate feature id from
 * indexing so query spend stays visible on its own.
 */
export async function embedQuery({ db, user, request = null, query }) {
  ...
  const cached = await db.collection('search_query_embeddings').findOne({ _id: key });
  if (cached?.vector?.length) return { ok: true, vector: cached.vector, costUsd: 0, cached: true };
  ...
  await db.collection('search_query_embeddings').updateOne(
    { _id: key },
    { $set: { vector: result.vectors[0], version: EMBEDDING_VERSION, updatedAt: new Date() } },
    { upsert: true },
  ).catch(() => {});

  return { ok: true, vector: result.vectors[0], costUsd: result.costUsd, cached: false };
}
