...
export const CANONICAL_REEL_MULTIPART_PART_SIZE_BYTES = 10 * 1024 * 1024;
export const CANONICAL_REEL_MULTIPART_URL_TTL_SEC = 10 * 60;
...
export function canonicalRenderProviderStatus(env = process.env, storageProvider = storage.active()) {
  const missing = [];
  if (!validHttpsUrl(env.CREATE_RENDER_PROVIDER_URL)) missing.push('CREATE_RENDER_PROVIDER_URL=https://...');
  if (!validHttpsUrl(env.CREATE_RENDER_CALLBACK_URL)) missing.push('CREATE_RENDER_CALLBACK_URL=https://...');
...
export function canonicalRenderCallbackUrl(env = process.env) {
  if (!validHttpsUrl(env.CREATE_RENDER_CALLBACK_URL)) return null;
  return String(env.CREATE_RENDER_CALLBACK_URL);
}
...
export async function buildCanonicalRenderProviderPayload({
  ...
  if (!validHttpsUrl(callbackUrl)) {
    const error = new Error('Canonical render callback URL must be a configured HTTPS endpoint.');
    error.code = 'render_callback_url_invalid';
    throw error;
  }
  ...
  if (artifact.canonicalManifest.music?.trackId) {
    ...
    music = {
      trackId: track.id,
      readUrl: track.audioUrl,
      contentHash: track.contentHash,
      checksumAlgorithm: track.checksumAlgorithm || null,
  ...
  return {
    ...
    output: {
      provider: 's3',
      mode: 'snapnext-controlled-multipart',
      contentType: 'video/mp4',
      maxBytes: CANONICAL_REEL_MAX_OUTPUT_BYTES,
      partSizeBytes: CANONICAL_REEL_MULTIPART_PART_SIZE_BYTES,
      uploadPlanStatus: 'upload_plan',
      ...decision.output,
    },
    callback: {
      url: callbackUrl,
