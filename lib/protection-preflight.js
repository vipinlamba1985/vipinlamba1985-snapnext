...
export async function preflightProtectionItem({ db, user, plan, usedBytes, item }) {
  ...
  const wantsMultipart = storage.active() === 's3' && mime.startsWith('video/') && size >= MULTIPART_THRESHOLD;
  if (wantsMultipart) {
    const existing = await db.collection('upload_reservations').findOne({
      userId: user.id,
      hash,
      bytes: size,
      status: 'reserved',
    ...
    if (existing?.multipartUploadId && existing?.storageKey) {
      return {
        localId,
        decision: 'ACCEPT',
        reservationId: existing.id,
        uploadMode: 'multipart',
        partSize: existing.multipartPartSize,
        totalParts: existing.multipartTotalParts,
        resumed: true,
      };
    }
  }
  ...
  if (wantsMultipart) {
    try {
      ...
      const expiresAt = new Date(Date.now() + MULTIPART_TTL_MS);
      await db.collection('upload_reservations').updateOne({ id: reservation.id }, { $set: {
        ...
        multipartPartSize: MULTIPART_PART_SIZE, multipartTotalParts: totalParts, expiresAt,
      } });
