...
export const DEFAULT_WEEKLY_AI_WALLETS_USD = Object.freeze({
  free: 0,
  plus: 0.08,
  pro: 0.18,
  family: 0.28,
...
export function weeklyAiWalletLimitUsd(planId) {
  ...
  const fallback = DEFAULT_WEEKLY_AI_WALLETS_USD[key] ?? 0;
  return Math.max(0, finite(process.env[envKey], fallback));
}

async function expireReservations(db, now = new Date()) {
  const expired = await db.collection('ai_wallet_reservations').find({
    status: 'reserved',
    expiresAt: { $lte: now },
  }).limit(200).toArray().catch(() => []);

  for (const reservation of expired) {
    const claimed = await db.collection('ai_wallet_reservations').findOneAndUpdate(
    ...
    await db.collection('ai_weekly_wallets').updateOne(
      ...
      { $inc: { reservedUsd: -Math.max(0, finite(reservation.reservedCostUsd, 0)) }, $set: { updatedAt: now } },
    ).catch(() => null);
  }
}
...
export async function getUserAiWalletSnapshot({ db, user, request = null, now = new Date() }) {
  ...
  const walletId = `${user.id}:${weekKey}`;
  ...
  await db.collection('ai_weekly_wallets').updateOne(
    { id: walletId },
    {
      ...
      $set: { plan, limitUsd: weeklyLimitUsd, updatedAt: now },
    },
    { upsert: true },
  );
...
export async function reserveUserAiWallet({ db, user, request = null, feature, estimatedCostUsd, metadata = {} }) {
  ...
  if (!snapshot.externalAiEnabled) {
    await recordWalletDecision(db, { userId: user.id, plan: snapshot.plan, feature, estimatedCostUsd: estimate, allowed: false, reason: 'external_ai_not_in_plan', metadata });
  ...
  const wallet = await db.collection('ai_weekly_wallets').findOneAndUpdate(
    {
      ...
      $expr: {
        $lte: [
          { $add: [{ $ifNull: ['$spentUsd', 0] }, { $ifNull: ['$reservedUsd', 0] }, estimate] },
          { $ifNull: ['$limitUsd', 0] },
        ],
      },
    },
  ...
  if (!wallet) {
    ...
    await recordWalletDecision(db, { userId: user.id, plan: latest.plan, feature, estimatedCostUsd: estimate, allowed: false, reason: 'weekly_wallet_exhausted', metadata });
    return { allowed: false, reason: 'weekly_wallet_exhausted', snapshot: latest };
  }

  const reservationId = uuidv4();
  await db.collection('ai_wallet_reservations').insertOne({
    id: reservationId,
  ...
  await recordWalletDecision(db, { userId: user.id, plan: snapshot.plan, feature, estimatedCostUsd: estimate, allowed: true, reason: 'within_weekly_wallet', metadata });
...
export async function settleUserAiWallet({ db, reservationId, actualCostUsd, metadata = {} }) {
  ...
  const reservation = await db.collection('ai_wallet_reservations').findOneAndUpdate(
    { id: reservationId, status: 'reserved' },
    { $set: { status: 'settled', actualCostUsd: actual, settlementMetadata: metadata, settledAt: now } },
    { returnDocument: 'before' },
  );
  ...
  await db.collection('ai_weekly_wallets').updateOne(
    { id: reservation.walletId },
    { $inc: { reservedUsd: -reserved, spentUsd: actual }, $set: { updatedAt: now } },
  );

  await db.collection('ai_wallet_ledger').insertOne({
    id: uuidv4(),
