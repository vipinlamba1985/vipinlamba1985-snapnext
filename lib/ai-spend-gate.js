...
export async function reserveExternalAiSpend({
  db,
  user,
  request = null,
  feature,
  ...
  estimatedCostUsd,
  essential = false,
  skipUserWallet = false,
  metadata = {},
}) {
  const approvedCostUsd = Math.max(0, Number(estimatedCostUsd) || 0);
  const prepaid = skipUserWallet || metadata?.billingPolicy === 'prepaid';
  const wallet = prepaid
    ? { allowed: true, reservationId: null, reason: 'prepaid_external_ai', snapshot: null }
    : await reserveUserAiWallet({ db, user, request, feature, estimatedCostUsd: approvedCostUsd, metadata });
  ...
  return {
    allowed: true,
    reason: prepaid ? 'prepaid_and_profit_guard_approved' : 'wallet_and_profit_guard_approved',
    billingPolicy: prepaid ? 'prepaid' : 'included',
    approvedCostUsd,
