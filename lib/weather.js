...
export function normalizeWeatherForecast(payload, context = {}) {
  ...
  const hourly = times
    .map((time, index) => {
      const hourlyCondition = describeWeatherCode(weatherCodes[index], isDay);
      return {
        time: String(time || ''),
        timeLabel: formatWeatherHour(time),
        temperature: numberOrNull(temperatures[index]),
        precipitationProbability: numberOrNull(precipitation[index]) ?? 0,
        condition: hourlyCondition.label,
        kind: hourlyCondition.kind,
      };
    })
  ...
  if (nextRain?.timeLabel) {
    summary = `Rain chance reaches ${Math.round(nextRain.precipitationProbability)}% around ${nextRain.timeLabel}.`;
