...
async function findParticipantConnection(db, userId, favoriteId) {
  const id = parseTrustedConnectionId(favoriteId);
  const favorite = await db.collection('favorites').findOne({ id });
  if (!favorite) throw new TrustedCircleApiError('Not found', 404, 'trusted_connection_not_found');
  if (favorite.requesterUserId !== userId && favorite.targetUserId !== userId) {
    throw new TrustedCircleApiError('Forbidden', 403, 'trusted_forbidden');
  }
  return favorite;
}

export async function listTrustedCircle(user) {
...
export async function inviteToTrustedCircle(user, body = {}) {
  const { email } = parseTrustedCircleInvite(body);
  const db = await getDb();
  const target = await db.collection('users').findOne({ email });
  if (!target) {
    throw new TrustedCircleApiError('No SnapNext user found with that email. Ask them to sign up first.', 404, 'trusted_user_not_found');
  }
  if (target.id === user.id) {
    throw new TrustedCircleApiError("You can't add yourself to your own trusted circle", 400, 'trusted_self_not_allowed');
  }
  ...
  if (existing?.status === 'blocked') throw new TrustedCircleApiError('Blocked', 403, 'trusted_blocked');
  if (existing?.status === 'accepted') return { ok: true, alreadyTrusted: true };
  ...
  try {
    const acceptUrl = `${process.env.NEXT_PUBLIC_APP_URL || process.env.NEXT_PUBLIC_BASE_URL || ''}/trusted-circle`;
    await sendEmail({
  ...
  await notify(db, {
    ...
    title: `${user.name} wants to join your trusted circle`,
    payload: { fromUserId: user.id, fromName: user.name },
  });
  return { ok: true };
}

export async function runTrustedCircleAction(user, favoriteId, requestedAction) {
  const action = parseTrustedCircleAction(requestedAction);
  const db = await getDb();
  ...
  if (action === 'accept') {
    if (favorite.targetUserId !== user.id || favorite.status !== 'pending') {
      throw new TrustedCircleApiError('Not allowed', 403, 'trusted_accept_not_allowed');
    }
    await db.collection('favorites').updateOne(
    ...
    await notify(db, {
      ...
      title: `${user.name} accepted your trusted circle invitation`,
      payload: { fromUserId: user.id, fromName: user.name },
    });
    return { ok: true };
  }
  ...
  if (action === 'decline') {
    if (favorite.targetUserId !== user.id) {
      throw new TrustedCircleApiError('Only the recipient can decline', 403, 'trusted_decline_not_allowed');
    }
    await db.collection('favorites').updateOne(
  ...
  if (action === 'cancel') {
    if (favorite.requesterUserId !== user.id || favorite.status !== 'pending') {
      throw new TrustedCircleApiError('Only sender can cancel pending', 403, 'trusted_cancel_not_allowed');
    }
...
export async function getTrustedCirclePermissions(user, favoriteId) {
  const db = await getDb();
  const favorite = await findParticipantConnection(db, user.id, favoriteId);
  if (favorite.status !== 'accepted') {
    throw new TrustedCircleApiError('Sharing permissions are available once the invitation is accepted.', 409, 'trusted_not_accepted');
  }
  const record = await db.collection('favorite_permissions').findOne({
    favoriteId: favorite.id,
    ownerUserId: user.id,
  });
  return { perms: { ...defaultTrustedCirclePermissions(), ...(record?.perms || {}) } };
}

export async function updateTrustedCirclePermissions(user, favoriteId, body = {}) {
  const db = await getDb();
  const favorite = await findParticipantConnection(db, user.id, favoriteId);
  if (favorite.status !== 'accepted') {
    throw new TrustedCircleApiError('Sharing permissions are available once the invitation is accepted.', 409, 'trusted_not_accepted');
