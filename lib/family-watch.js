...
export function publicFamilyWatchControllerState(session) {
  ...
  const transport = String(session.transport || 'browser');
  const isBrowserPairing = transport === 'browser';
  const showVerification = isBrowserPairing && ['claimed', 'approved', 'ended'].includes(session.status);
  return {
    id: session.id,
    status: session.status,
    transport,
    pairCode: isBrowserPairing ? session.pairCode : null,
    ...
    claimExpiresAt: isBrowserPairing && session.claimExpiresAt ? new Date(session.claimExpiresAt).toISOString() : null,
    expiresAt: session.expiresAt ? new Date(session.expiresAt).toISOString() : null,
    claimedAt: isBrowserPairing && session.claimedAt ? new Date(session.claimedAt).toISOString() : null,
    approvedAt: session.approvedAt ? new Date(session.approvedAt).toISOString() : null,
    viewerReadyAt: isBrowserPairing && session.viewerReadyAt ? new Date(session.viewerReadyAt).toISOString() : null,
  };
}
