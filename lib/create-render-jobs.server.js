...
export async function dispatchCanonicalRenderJob({
  ...
  if (canonicalRenderJobDeadlineExpired(job)) {
    const failed = await permanentlyFailDispatch({
      ...
      message: 'Canonical render job exceeded its bounded execution deadline before dispatch.',
    });
    return { accepted: false, permanent: true, reason: 'render_job_deadline_exceeded', job: failed };
  }
