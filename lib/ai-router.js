...
export async function preflightAiRequest({ db, user, feature, prompt, media, multiplier = 1, request }) {
  ...
  if (!isSuperUser(user) && dailyUsed + entitlement.credits > entitlement.limits.dailyCredits) {
  ...
  if (!isSuperUser(user) && monthlyUsed + entitlement.credits > entitlement.limits.monthlyCredits) {
...
export async function getAiUsageSummary({ db, user }) {
  if (!isSuperUser(user)) return structuredError('feature_not_available', 'AI analytics are available to Super User only.', 403);
