...
import { getPlan, isSuper } from '@/lib/plans';
...
function planKey(user) {
  ...
  if (current === 'pro' || current === 'plus' || current === 'free' || current === 'family') return current;
  return getPlan(current)?.id || 'free';
}
...
export function getAiEntitlement(user, feature, multiplier = 1) {
...
export async function preflightAiRequest({ db, user, feature, prompt, media, multiplier = 1, request }) {
  ...
  const entitlement = getAiEntitlement(user, feature, multiplier);
  ...
  if (!isSuper(user) && dailyUsed + entitlement.credits > entitlement.limits.dailyCredits) {
  ...
  if (!isSuper(user) && monthlyUsed + entitlement.credits > entitlement.limits.monthlyCredits) {
...
export async function getAiUsageSummary({ db, user }) {
  if (!isSuper(user)) return structuredError('feature_not_available', 'AI analytics are available to Super User only.', 403);
