...
export async function uploadOneProtectedItem(item, decision, onUpdate) {
  ...
  try {
    const result = decision.uploadMode === 'direct' && decision.uploadUrl
      ? await uploadProtectedDirect(item, decision, progress)
      : await uploadProtectedViaServer(item, decision.reservationId, progress);
