...
export async function uploadOneProtectedItem(item, decision, onUpdate) {
  ...
  try {
    ...
    // Backup completion never waits on local classification. The work already
    // started beside the upload and persists when it settles; if navigation
    // interrupts it, the bounded catch-up queue sees the missing version later.
