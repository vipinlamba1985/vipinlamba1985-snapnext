...
export async function uploadOneProtectedItem(item, decision, onUpdate) {
  ...
  try {
    ...
    onUpdate(item.localId, { status: decision.uploadMode === 'multipart' ? 'paused' : 'failed', progress: 0, error: error?.message || 'Upload interrupted' });
    return decision.uploadMode === 'multipart' ? 'paused' : 'failed';
  }
}
