...
export async function estimateAiEconomy({ db, user, feature, qualityMode = QUALITY_MODES.BALANCED, prompt = '', input = {} }) {
  ...
  const profitGate = isSuperUser(user) || estimatedCostUsd <= Math.max(estimatedAiBudget, 0.02);
  const allowed = isSuperUser(user) || (monthlyRemaining >= requiredCredits && dailyRemaining >= requiredCredits && profitGate);
