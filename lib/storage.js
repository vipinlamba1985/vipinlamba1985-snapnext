...
export const STORAGE_CONFIG = {
  ...
  productionReady: !IS_PRODUCTION || ACTIVE === 's3',
  signedTtl: SIGNED_TTL,
  maxUploadBytes: MAX_UPLOAD_BYTES,
};

function productionStorageError() {
  const error = new Error('Production storage requires STORAGE_PROVIDER=s3. Local disk storage is disabled in production.');
  error.code = 'production_storage_requires_s3';
  return error;
}

function assertProductionProvider(provider = ACTIVE) {
  if (IS_PRODUCTION && provider !== 's3') throw productionStorageError();
}
...
const local = {
  name: 'local',
  get ready() { return !IS_PRODUCTION; },
  ...
  health() {
    return IS_PRODUCTION
      ...
      : { name: 'local', ready: true, uploadDir: UPLOAD_DIR };
  },
};
