...
const ENFORCE_DURABLE_S3 = process.env.VERCEL_ENV === 'production'
  || ['1', 'true', 'yes', 'on'].includes(String(process.env.REQUIRE_DURABLE_S3 || '').toLowerCase());
...
export const STORAGE_CONFIG = {
  ...
  durableS3Required: ENFORCE_DURABLE_S3,
  productionReady: !ENFORCE_DURABLE_S3 || ACTIVE === 's3',
  signedTtl: SIGNED_TTL,
  maxUploadBytes: MAX_UPLOAD_BYTES,
};

function productionStorageError() {
  const error = new Error('This deployment requires STORAGE_PROVIDER=s3. Local disk storage is disabled on ephemeral production hosting.');
  error.code = 'production_storage_requires_s3';
  return error;
}

function assertProductionProvider(provider = ACTIVE) {
  if (ENFORCE_DURABLE_S3 && provider !== 's3') throw productionStorageError();
}
...
function assertExpectedSize(actual, expected, provider, storageKey) {
  if (!Number.isFinite(actual)) throw new Error(`${provider} verification did not return a valid size for ${storageKey}`);
  if (actual !== expected) throw new Error(`${provider} verification size mismatch for ${storageKey}: expected ${expected}, received ${actual}`);
}

const local = {
  name: 'local',
  get ready() { return !ENFORCE_DURABLE_S3; },

  async save({ userId, fileId, buffer, ext }) {
    assertProductionProvider('local');
  ...
  async getReadUrl() { assertProductionProvider('local'); return null; },
  health() {
    return ENFORCE_DURABLE_S3
      ? { name: 'local', ready: false, uploadDir: UPLOAD_DIR, lastError: productionStorageError().message }
      : { name: 'local', ready: true, uploadDir: UPLOAD_DIR, persistentVolumeExpected: IS_PRODUCTION };
  },
};
...
function pick(provider) {
  const selected = provider && PROVIDERS[provider] ? PROVIDERS[provider] : (PROVIDERS[ACTIVE] || local);
  assertProductionProvider(selected.name);
  return selected;
}
...
export const storage = {
  ...
  validateBeforeSave({ size }) {
    assertProductionProvider(ACTIVE);
  ...
  async health() {
    return {
      ...
      durableS3Required: ENFORCE_DURABLE_S3,
      productionReady: !ENFORCE_DURABLE_S3 || ACTIVE === 's3',
      providers: { local: local.health(), s3: await s3.health() },
    };
  },
};
