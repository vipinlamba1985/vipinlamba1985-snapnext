...
export const STORAGE_CONFIG = { active: ACTIVE, signedTtl: SIGNED_TTL, maxUploadBytes: MAX_UPLOAD_BYTES };
...
const local = {
  ...
  async read({ storageKey }) { return fs.readFile(path.join(UPLOAD_DIR, storageKey)); },
  async delete({ storageKey }) { try { await fs.unlink(path.join(UPLOAD_DIR, storageKey)); } catch {} },
...
async function getS3() {
  ...
  _s3Client = new S3Client({ region: process.env.AWS_REGION, credentials: { accessKeyId: process.env.AWS_ACCESS_KEY_ID, secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY } });
...
const s3 = {
  ...
  async save({ userId, fileId, buffer, ext, name, mime }) {
    if (buffer.length > MAX_UPLOAD_BYTES) throw new Error(`File exceeds max size (${process.env.MAX_UPLOAD_SIZE_MB || 500} MB). Use multipart upload.`);
    const { client } = await getS3();
    const { PutObjectCommand } = await import('@aws-sdk/client-s3');
    const safeName = sanitizeFileName(name || `${fileId}.${ext || 'bin'}`);
    const key = `users/${userId}/media/${fileId}/${safeName}`;
    await client.send(new PutObjectCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: key, Body: buffer, ContentType: mime || 'application/octet-stream' }));
  ...
  async delete({ storageKey }) {
    ...
    } catch (e) { console.error('[storage:s3] delete failed', e?.message); }
  },
  ...
  async getUploadUrl({ userId, fileId, name, mime, expiresSec = 600 }) {
    const { client, presign } = await getS3();
    const { PutObjectCommand } = await import('@aws-sdk/client-s3');
    const key = `users/${userId}/media/${fileId}/${sanitizeFileName(name || fileId)}`;
    const url = await presign(client, new PutObjectCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: key, ContentType: mime || 'application/octet-stream' }), { expiresIn: expiresSec });
  ...
  async createMultipartUpload({ userId, fileId, name, mime }) {
    ...
    const key = `users/${userId}/media/${fileId}/${sanitizeFileName(name || fileId)}`;
    const res = await client.send(new CreateMultipartUploadCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: key, ContentType: mime || 'application/octet-stream' }));
    return { storageKey: key, uploadId: res.UploadId };
  },
  async signUploadPartUrl({ storageKey, uploadId, partNumber, expiresSec = 900 }) {
    const { client, presign } = await getS3();
    const { UploadPartCommand } = await import('@aws-sdk/client-s3');
    return presign(client, new UploadPartCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: storageKey, UploadId: uploadId, PartNumber: partNumber }), { expiresIn: expiresSec });
  },
  async completeMultipartUpload({ storageKey, uploadId, parts }) {
    const { client } = await getS3();
    const { CompleteMultipartUploadCommand } = await import('@aws-sdk/client-s3');
    return client.send(new CompleteMultipartUploadCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: storageKey, UploadId: uploadId, MultipartUpload: { Parts: parts } }));
  },
  async abortMultipartUpload({ storageKey, uploadId }) {
    const { client } = await getS3();
    const { AbortMultipartUploadCommand } = await import('@aws-sdk/client-s3');
    return client.send(new AbortMultipartUploadCommand({ Bucket: process.env.AWS_S3_BUCKET, Key: storageKey, UploadId: uploadId }));
  },
  async health() {
    const missing = s3EnvMissing();
    ...
    } else lastError = `Missing env: ${missing.join(', ')}`;
    return { name: 's3', ready: ok, bucketReachable, lastError, region: process.env.AWS_REGION || null };
  },
};

const PROVIDERS = { local, s3 };
function pick(p) { return p && PROVIDERS[p] ? PROVIDERS[p] : (PROVIDERS[ACTIVE] || local); }

export const storage = {
  config: STORAGE_CONFIG,
  active() { return ACTIVE; },
  validateBeforeSave({ size }) { if (size > MAX_UPLOAD_BYTES) throw new Error(`File too large. Max ${process.env.MAX_UPLOAD_SIZE_MB || 500} MB. Larger files require multipart upload.`); },
  ...
  async getReadUrl({ provider, storageKey, ...rest }) { return pick(provider).getReadUrl?.({ storageKey, ...rest }) || null; },
  async getUploadUrl(args) { if (ACTIVE !== 's3') throw new Error('Direct upload URLs require STORAGE_PROVIDER=s3'); return s3.getUploadUrl(args); },
  async createMultipartUpload(args) { if (ACTIVE !== 's3') throw new Error('Multipart upload requires STORAGE_PROVIDER=s3'); return s3.createMultipartUpload(args); },
  async signUploadPartUrl(args) { if (ACTIVE !== 's3') throw new Error('Multipart upload requires STORAGE_PROVIDER=s3'); return s3.signUploadPartUrl(args); },
  async completeMultipartUpload(args) { if (ACTIVE !== 's3') throw new Error('Multipart upload requires STORAGE_PROVIDER=s3'); return s3.completeMultipartUpload(args); },
  async abortMultipartUpload(args) { if (ACTIVE !== 's3') throw new Error('Multipart upload requires STORAGE_PROVIDER=s3'); return s3.abortMultipartUpload(args); },
  async health() { return { active: ACTIVE, providers: { local: local.health(), s3: await s3.health().catch((e) => ({ name: 's3', ready: false, lastError: e?.message })) } }; },
};
