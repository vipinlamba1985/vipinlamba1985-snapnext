...
export async function indexMediaFaces({ db, userId, item }) {
  if (!eligibleForPeopleIndex(item)) return { status: 'skipped', reason: 'not_eligible', faces: 0, clusters: [] };

  const existing = item.peopleIntelligence;
  if (existing?.version === PEOPLE_INTELLIGENCE_VERSION && existing?.status === 'completed') {
    return { status: 'already_indexed', faces: existing.faceIds?.length || 0, clusters: existing.clusterIds || [] };
  }
...
export async function rebuildPeopleIntelligence({ db, userId, limit = 12, reset = false }) {
  ...
  const candidates = await db.collection('media').find({
    ...
    $or: [
      ...
      { 'peopleIntelligence.status': { $ne: 'completed' } },
    ],
  ...
  const remaining = await db.collection('media').countDocuments({
    ...
    $or: [
      { 'peopleIntelligence.version': { $ne: PEOPLE_INTELLIGENCE_VERSION } },
      { 'peopleIntelligence.status': { $nin: ['completed', 'failed'] } },
    ],
  });
