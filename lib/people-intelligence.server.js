...
export async function indexMediaFaces({ db, userId, item }) {
  ...
  // Large group photos stay fully available in the library but must not create
  // or strengthen identities: clustering a crowd shot spawns a cluster per
  // guest and inflates every distinct-photo count it touches. Exclusion happens
  // here, before any cluster write, rather than being hidden at read time.
