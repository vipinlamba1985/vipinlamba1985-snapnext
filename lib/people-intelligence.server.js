...
async function resetCollection(userId) {
  ...
  const collectionId = peopleCollectionId(userId);
  try { await client.send(new DeleteCollectionCommand({ CollectionId: collectionId })); }
...
async function ensureRekognitionUser(collectionId, clusterId) {
  ...
  const userId = rekognitionUserId(clusterId);
  try { await client.send(new CreateUserCommand({ CollectionId: collectionId, UserId: userId })); }
  catch (error) { if (error?.name !== 'ResourceAlreadyExistsException' && error?.name !== 'ConflictException') throw error; }
  return userId;
}

async function associateFace(collectionId, userId, faceId) {
  const client = await getClient();
...
async function findExistingCluster(db, userId, collectionId, faceId) {
  ...
  for (const match of matches) {
    ...
    const cluster = await db.collection('person_clusters').findOne({ userId, rekognitionUserId: awsUserId });
    if (cluster) {
...
async function upsertCluster(db, { userId, clusterId, rekognitionUserId: awsUserId, mediaId, faceId, faceBox, quality, similarity, verificationStatus }) {
  const now = new Date();
  await db.collection('person_clusters').updateOne(
    { userId, clusterId },
    {
      $setOnInsert: { userId, clusterId, displayName: null, status: 'discovered', createdAt: now },
      $set: {
        rekognitionUserId: awsUserId,
        updatedAt: now,
        verificationStatus: verificationStatus || 'suggested',
        bestSimilarity: Math.max(Number(similarity || 0), 0),
      },
...
export async function rebuildPeopleIntelligence({ db, userId, limit = 12, reset = false }) {
  ...
  } else await ensureCollection(userId);
