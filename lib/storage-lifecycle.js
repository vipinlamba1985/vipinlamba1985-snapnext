 ...
 *
 * `originals/` is the only prefix moved. Thumbnails and derivatives stay in
 * Standard permanently — they are small, they are read constantly, and moving
 * them would add a retrieval charge to ordinary browsing for almost no saving.
 */
export function mediaLifecycleConfiguration({ originalsPrefix = 'originals/', derivativesPrefix = 'thumbs/' } = {}) {
  return {
    Rules: [
      {
