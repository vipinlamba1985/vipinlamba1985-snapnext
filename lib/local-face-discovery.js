...
function clusterId() {
  return crypto.randomUUID?.() || `person-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
function addToClusters(clusters, face) {
  ...
  if (!best || bestScore < 0.78) {
    clusters.push({ id: clusterId(), centroid: face.descriptor, faces: [face], preview: face.preview, count: 1 });
    return;
  }
