...
/**
 * A photo with more usable faces than a family-sized group must not create or
 * strengthen identities. Applied at index time (before any cluster write) and
 * again at read time, so both paths agree on the same boundary.
 */
export function isLargeGroupPhoto(usableFaceCount) {
  return count(usableFaceCount) > MAX_FAMILY_SIZED_FACE_COUNT;
}
...
export function classifyPersonMedia(item = {}, { selectedClusterId = '', activeClusterIds = [] } = {}) {
  ...
  const confirmedClusterIds = uniqueStrings((Array.isArray(item.userConfirmedPeople) ? item.userConfirmedPeople : []).map((entry) => entry?.clusterId));
  const selected = String(selectedClusterId || '').trim();
  const active = new Set(uniqueStrings(activeClusterIds));
  // Favourite People does not retain ordinary-photo AWS face ids. Preserve the
  // real local face-count signal so group-photo UX stays accurate after those
  // temporary cloud vectors are deleted.
  const detectedFaceCount = Math.max(1, count(intelligence.detectedFaceCount), faceIds.length, clusterIds.length);
  ...
  return {
    ...
    bestEligible,
    userConfirmed: Boolean(selected && confirmedClusterIds.includes(selected)),
    aiRecognized: Boolean(selected && clusterIds.includes(selected)),
  };
}
