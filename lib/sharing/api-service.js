...
import { canViewOwnersResource, getTrustedLink } from '@/lib/trusted-circle/links';
...
export async function sharePhotos(user, body = {}) {
  ...
  if (!link) throw new SharingApiError('Not in your trusted circle', 403, 'sharing_trusted_circle_required');
...
export async function runSharedAlbumAction(user, albumId, requestedAction, body = {}) {
  ...
  if (action === 'invite') {
    const link = await getTrustedLink(db, user.id, input.favoriteUserId);
    if (!link) throw new SharingApiError('Not in your trusted circle', 403, 'sharing_trusted_circle_required');
