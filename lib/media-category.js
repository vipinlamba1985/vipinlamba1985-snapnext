...
export function isScreenshotMedia(item = {}) {
  ...
  if (confidence >= 0.6 && ['screenshot', 'screen_capture', 'screen capture'].includes(contentType)) return true;

  const name = String(item.name || '').trim();
  if (SCREENSHOT_FILENAME.test(name)) return true;
  if (IOS_SCREENSHOT_FILENAME.test(name) && /screen|app|website|message|menu|button|interface|\bui\b/.test(analysisText(item))) return true;
  return false;
}
...
export function screenshotType(item = {}) {
  ...
  // Preserve old user behavior: screenshots manually moved to Docs become Docs screenshots.
