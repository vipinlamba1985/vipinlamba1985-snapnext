...
const OWNER_EMAILS = new Set(
  String(process.env.SNAPNEXT_OWNER_EMAILS || 'vipin.lamba1985@gmail.com')
    .split(',')
    .map((email) => email.trim().toLowerCase())
    .filter(Boolean),
);

function ownerOverride(email) {
  const normalized = String(email || '').toLowerCase();
  return OWNER_EMAILS.has(normalized) ? { role: 'admin', plan: 'super_user' } : null;
}

async function syncOwnerSupabaseMetadata(supabaseUser, owner) {
  ...
  try {
    ...
    console.error('[auth] Owner Supabase metadata sync failed:', error?.message);
  }
}
...
export function normalizeSupabaseUser(supabaseUser, existing = {}) {
  ...
  const appMetadata = supabaseUser?.app_metadata || {};
  const owner = ownerOverride(email);
  return {
    id: existing?.id,
    supabaseUserId: supabaseUser?.id,
    email,
    name: existing?.name || metadata.name || metadata.full_name || email.split('@')[0] || 'User',
    plan: owner?.plan || existing?.plan || appMetadata.plan || metadata.plan || 'free',
    role: owner?.role || existing?.role || appMetadata.role || metadata.role || 'user',
...
export async function syncSupabaseUserToAppUser(db, supabaseUser) {
  ...
  const owner = ownerOverride(email);
  await syncOwnerSupabaseMetadata(supabaseUser, owner);
  const name = existing?.name || metadata.name || metadata.full_name || email.split('@')[0] || 'User';
  const plan = owner?.plan || existing?.plan || appMetadata.plan || metadata.plan || 'free';
  const role = owner?.role || existing?.role || appMetadata.role || metadata.role || 'user';
...
export async function getUserFromRequest(request) {
  ...
  try {
    ...
    if (payload?.userId) {
      ...
      if (user) {
        const owner = ownerOverride(user.email);
        const patched = owner ? { ...user, ...owner } : user;
