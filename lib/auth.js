...
export function normalizeSupabaseUser(supabaseUser, existing = {}) {
  ...
  return {
    ...
    plan: owner?.plan || existing?.plan || metadata.plan || 'free',
    role: owner?.role || existing?.role || metadata.role || 'user',
...
export async function syncSupabaseUserToAppUser(db, supabaseUser) {
  ...
  const plan = owner?.plan || existing?.plan || metadata.plan || 'free';
  const role = owner?.role || existing?.role || metadata.role || 'user';
