...
const INVALIDATABLE_RENDER_STATUSES = Object.freeze(['rendering', 'pending_validation', 'ready']);
...
export async function findCachedCanonicalRender({ db, userId, manifestHash }) {
  ...
  return db.collection('render_artifacts').findOne({
...
async function deleteArtifactOutput({ artifact, deleteStored = deleteStoredMediaStrict }) {
  if (!artifact?.storageKey) return;
  await deleteStored({ provider: artifact.provider || 's3', storageKey: artifact.storageKey });
}

async function markArtifactStale({ db, artifact, reason, details = null, deleteStored = deleteStoredMediaStrict }) {
  try {
    ...
    const cleanupError = new Error('Derived render artifact could not be removed.');
...
export async function prepareCanonicalRender({
  ...
  const mediaDeletionGeneration = await getMediaDeletionGeneration({ db, userId: user.id });
  ...
  let claimed = false;
  if (existing && ['failed', 'stale_source', 'deletion_failed'].includes(existing.status)) {
  ...
  if (!claimed) {
    await Promise.all([
      releaseCanonicalRenderQuota({ db, reservationId: quota.reservationId, reason: 'duplicate_manifest_claim' }),
      releaseProductSpendReservation({ db, reservationId: cost.reservationId, reason: 'duplicate_manifest_claim' }),
    ]);
    const winner = await db.collection('render_artifacts').findOne({ _id: artifactId, userId: user.id });
    return {
...
export async function finalizeCanonicalRender({
  ...
  const sources = artifact.sourceMediaIds.map(mediaId => ({ mediaId, contentHash: artifact.sourceContentHashes?.[mediaId] || '' }));
...
export async function failCanonicalRender({ db, userId, artifactId, error, deleteStored = deleteStoredMediaStrict }) {
...
export async function invalidateRenderArtifactsForSources({
  ...
  if (failures.length) {
    const error = new Error('One or more derived render artifacts could not be removed.');
