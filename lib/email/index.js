...
import { DEFAULT_SENDER, replyToAddress, resolveSender } from './senders';
...
async function sendWithResend({ to, subject, html, text, replyTo, from }) {
  ...
  try {
    if (PROVIDER === 'resend' && process.env.RESEND_API_KEY) {
      result = await sendWithResend({
        ...
        html: rendered.html,
        text: rendered.text,
        // Warm mail comes from hello@, account and security mail from support@,
        // but a reply to either always reaches the monitored inbox.
        from: resolveSender(def.sender || DEFAULT_SENDER),
