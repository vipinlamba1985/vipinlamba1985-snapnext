...
/**
 * Runs protection preflight in bounded batches while keeping enough information
 * to release reservations if a later batch fails or the selection becomes stale.
 * The injected functions keep the race/cleanup policy unit-testable without a
 * browser or Next.js runtime.
 */
export async function runProtectionDecisionBatches(items = [], {
  ...
  try {
    for (let index = 0; index < items.length; index += batchSize) {
      ...
      onBatch?.(batchDecisions);

      // A response can arrive after the user selected a different batch. Record
      // its reservation ids first, then reject it so cleanup can release them.
      if (cancelled(isCurrent)) throw new ProtectionPreparationCancelledError();
