  ...
  await db.collection('restoration_wallets').updateOne(
    { userId },
    {
      $setOnInsert: {
        ...
        refundedUsedUnits: 0,
        activeRestorationReservationId: null,
        grantIds: [],
        createdAt: now,
      },
      $set: { updatedAt: now },
...
export async function getRestorationWallet(db, userId) {
  ...
  return {
    ...
    active: Boolean(wallet?.activeRestorationReservationId),
...
export async function reserveRestorationUnits({ db, userId, reservationId, units, metadata = {} }) {
  ...
  const wallet = await db.collection('restoration_wallets').findOneAndUpdate(
    {
      userId,
      availableUnits: { $gte: required },
      $or: [
        { activeRestorationReservationId: null },
        { activeRestorationReservationId: { $exists: false } },
      ],
    },
    {
      $inc: { availableUnits: -required, reservedUnits: required },
      $set: { activeRestorationReservationId: reservationId, updatedAt: now },
    },
    { returnDocument: 'after' },
  );

  if (!wallet) {
    const current = await db.collection('restoration_wallets').findOne({ userId });
    const active = Boolean(current?.activeRestorationReservationId);
    return {
      ok: false,
      code: active ? 'restoration_in_progress' : 'restoration_credits_required',
      wallet: await getRestorationWallet(db, userId),
  ...
  try {
    ...
    await db.collection('restoration_wallets').updateOne(
      { userId, activeRestorationReservationId: reservationId },
      {
        $inc: { availableUnits: required, reservedUnits: -required },
        $unset: { activeRestorationReservationId: '' },
...
async function clearActiveReservation(db, userId, reservationId, now) {
  await db.collection('restoration_wallets').updateOne(
    { userId, activeRestorationReservationId: reservationId },
    { $unset: { activeRestorationReservationId: '' }, $set: { updatedAt: now } },
  );
}
...
export async function settleRestorationUnits({ db, reservationId, jobId = null }) {
  ...
  await clearActiveReservation(db, reservation.userId, reservationId, now);
