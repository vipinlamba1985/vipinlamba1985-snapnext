...
import { isRestorationProviderReady } from './provider.js';
...
export async function createRestorationPackCheckout({ user, packId }) {
  ...
  if (!process.env.STRIPE_WEBHOOK_SECRET) {
    const error = new Error('Restoration checkout is paused until verified Stripe webhooks are configured.');
    error.code = 'restoration_webhook_not_configured';
    throw error;
  }
  const currency = restorationCurrency();
  if (currency !== 'usd') {
    const error = new Error('The Restoration MVP currently supports USD packs only.');
    error.code = 'restoration_currency_not_supported';
    throw error;
  }
