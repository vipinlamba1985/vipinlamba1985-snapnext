...
import { pendingMagicEligibilityFields } from './magic-manifest.js';
import { markMagicManifestDirty } from './magic-manifest.server.js';
...
/**
 * Builds the library search pattern.
 *
 * A bare substring match meant "car" returned photos tagged "scarf" and
 * "oscar", because the term matched in the middle of an unrelated word.
 * Anchoring to a word start fixes that while still letting "birth" find
 * "birthday", which is what people expect from a search box.
 *
 * `\b` in the database regex is ASCII-only, so anchoring a term containing
 * Devanagari, Arabic or CJK characters would stop it matching at all. Those
 * terms keep the original substring behaviour rather than silently breaking
 * search for anyone not typing in Latin script.
 */
export function buildSearchPattern(value = '') {
  const escaped = escapeSearchPattern(value);
  if (!escaped) return '';
  return /^[\w-]+$/.test(escaped) ? `\\b${escaped}` : escaped;
}
...
export async function listUserMedia({ db, userId, filter = 'all', query = '', limit = 500 }) {
  ...
  if (search) {
    ...
    mongoQuery.$or = searchable.map(field => ({ [field]: { $regex: pattern, $options: 'i' } }));
  }
...
function dirtyReasonForAction(action) {
...
export async function applyMediaAction({ db, userId, id, action }) {
  ...
  if (action === 'favorite') {
    ...
    await coordinatePermanentMediaDeletion({ db, userId, docs: [doc], reason: 'library_single_delete' });
  }

  await markMagicManifestDirty(db, userId, dirtyReasonForAction(action));
  return { ok: true };
}
...
export async function applyBulkMediaAction({ db, userId, body = {} }) {
  ...
  if (action === 'trash') {
    ...
    await coordinatePermanentMediaDeletion({ db, userId, docs, reason: 'library_bulk_delete' });
  }
...
export async function createTextMedia({ db, userId, body = {} }) {
  ...
  await markMagicManifestDirty(db, userId, 'asset_added');
