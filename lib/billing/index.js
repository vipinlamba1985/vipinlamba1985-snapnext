    ...
    try {
      switch (event.type) {
        ...
        case 'customer.subscription.deleted': {
          ...
          const userId = await resolveExistingUserIdFromSubscription(db, sub);
          if (userId) await downgradeUser(db, userId, { reason: 'subscription_deleted', subscriptionId: sub.id });
          break;
        }
        case 'invoice.payment_failed': {
          const inv = event.data.object;
          const userId = await resolveExistingUserIdFromCustomer(db, inv.customer);
          if (userId) {
        ...
        case 'charge.refunded': {
          ...
          const userId = await resolveExistingUserIdFromCustomer(db, charge.customer);
...
async function applySubscription(db, sub) {
  const userId = await resolveExistingUserIdFromSubscription(db, sub);
  if (!userId) {
    console.warn('[billing] ignoring subscription event for missing/deleted user, customer=', sub.customer);
    return;
  }
...
async function downgradeUser(db, userId, meta = {}) {
  const user = await db.collection('users').findOne({ id: userId }, { projection: { id: 1 } });
  if (!user) return;
  const now = new Date();
