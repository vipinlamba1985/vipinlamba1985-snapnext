...
const mockProvider = {
  ...
  async createCheckoutSession({ user, planId, interval = 'monthly' }) {
    if (IS_PROD && ACTIVE === 'mock') {
      throw new Error('Mock billing cannot run in production. Set BILLING_PROVIDER=stripe.');
    }
    ...
    await db.collection('subscriptions').updateOne(
      { userId: user.id },
      { $set: {
          userId: user.id, provider: 'mock', status: 'active',
          plan: plan.id, billingInterval: interval,
    ...
    await logBillingEvent({ provider: 'mock', type: 'mock.upgrade', userId: user.id, status: 'processed', payload: { planId, interval } });
...
const stripeProvider = {
  ...
  async createCheckoutSession({ user, planId, interval = 'monthly' }) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: { metadata: { userId: user.id, planId: plan.id, interval } },
      success_url: `${APP_URL}/billing?success=1&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${APP_URL}/billing?cancelled=1`,
      allow_promotion_codes: true,
    });

    await logBillingEvent({ provider: 'stripe', type: 'checkout.session.created', userId: user.id, status: 'processed', payload: { sessionId: session.id, planId, interval } });
  ...
  /** Process a verified Stripe webhook event. Idempotent on event.id. */
  async handleWebhook({ event }) {
    ...
    const eventDoc = {
      id: existing?.id || uuidv4(), provider: 'stripe', eventId: event.id,
      type: event.type, processedAt: null, status: 'received',
      payload: summarize(event), createdAt: new Date(),
    };
    ...
    try {
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (userId && s.customer) {
            await db.collection('users').updateOne({ id: userId }, { $set: { stripeCustomerId: s.customer } });
          }
          // Activation of plan happens via subscription.created/updated.
          break;
        }
        ...
        case 'customer.subscription.deleted': {
          ...
          if (userId) {
            ...
            await db.collection('subscriptions').updateOne(
              { userId },
              { $set: { status: 'canceled', plan: 'free', updatedAt: new Date(), canceledAt: new Date() } },
              { upsert: true },
            );
          }
          break;
        ...
        case 'invoice.payment_failed': {
          ...
          if (userId) {
            await db.collection('subscriptions').updateOne(
              { userId },
              { $set: { status: 'past_due', latestInvoiceId: inv.id, updatedAt: new Date() } },
              { upsert: true },
            );
          }
          break;
        ...
        case 'invoice.payment_succeeded': {
          ...
          if (userId) {
            await db.collection('subscriptions').updateOne(
              { userId },
              { $set: { latestInvoiceId: inv.id, status: 'active', updatedAt: new Date() } },
              { upsert: true },
            );
          }
          break;
      ...
      await db.collection('billing_events').updateOne(
        { eventId: event.id }, { $set: { status: 'processed', processedAt: new Date() } },
      );
      return { ok: true };
    } catch (e) {
      await db.collection('billing_events').updateOne(
        { eventId: event.id }, { $set: { status: 'error', error: e?.message || String(e), processedAt: new Date() } },
      );
      throw e;
    }
  },
...
async function applySubscription(db, sub) {
  ...
  const now = Date.now();
  const update = {
    userId, provider: 'stripe', stripeCustomerId: sub.customer, stripeSubscriptionId: sub.id,
    status: sub.status, plan: planId, billingInterval: interval, priceId,
  ...
  await db.collection('subscriptions').updateOne(
    { userId }, { $set: update, $setOnInsert: { id: uuidv4(), createdAt: new Date() } }, { upsert: true },
  );

  // Activate plan only on healthy subscription statuses; revert to free on non-active.
  const userPlanUpdate = (['active', 'trialing'].includes(sub.status)) ? planId : 'free';
  await db.collection('users').updateOne({ id: userId }, { $set: { plan: userPlanUpdate, stripeCustomerId: sub.customer } });
}
...
async function logBillingEvent({ provider, type, userId = null, status = 'processed', payload = {}, error = null }) {
  try {
    ...
    await db.collection('billing_events').insertOne({
      id: uuidv4(), provider, type, userId, status, payload, error,
...
function activeProvider() {
  if (ACTIVE === 'stripe') return stripeProvider;
  return mockProvider;
}
...
export const billing = {
  ...
  async health() {
    ...
    return {
      active: ACTIVE,
      ready,
      stripeKey: stripeKeyMasked,
      webhookSecret: !!process.env.STRIPE_WEBHOOK_SECRET,
      publishableKey: !!process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY,
      priceIds,
      hasAnyPrices: hasAnyStripePrices(),
      missing,
      production: IS_PROD,
    };
  },
