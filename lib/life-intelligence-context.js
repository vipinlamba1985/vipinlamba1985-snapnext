import { buildContextualSearchGroups, searchMemoryBrain } from '@/lib/memory-brain';
...
export async function buildLifeIntelligenceContext(db, userId, query, options = {}) {
  ...
  const search = searchMemoryBrain(items, resolved.expandedQuery, {
    limit: Math.max(matchLimit, 30),
    intentQuery: query,
    requiredTermGroups: buildContextualSearchGroups(query, resolved.matchedRelationships),
  });
