...
export const SOCIAL_CONNECTORS = {
  youtube: { id: 'youtube', label: 'YouTube', status: 'available', canAuthorize: true, canImportFollowing: true, canReadUpdates: true, scope: 'https://www.googleapis.com/auth/youtube.readonly' },
...
function recencyScore(publishedAt) {
  ...
  if (ageHours <= 12) return 25; if (ageHours <= 24) return 20; if (ageHours <= 72) return 12; if (ageHours <= 168) return 6; return 0;
}
...
export async function syncYouTubeUpdates({ db, userId, connection }) {
  ...
  for (const source of sources) {
    ...
    for (const item of playlistData.items || []) {
      ...
      const mutable = { userId, sourceId: source.id, circleId: source.circleId, platform: 'youtube', externalUpdateId: `youtube:${videoId}`, type: 'video', title: snippet.title || `${source.displayName} video`, excerpt: snippet.description ? snippet.description.slice(0, 280) : '', thumbnailUrl: snippet.thumbnails?.medium?.url || snippet.thumbnails?.default?.url || null, externalUrl: `https://www.youtube.com/watch?v=${videoId}`, publishedAt: new Date(snippet.publishedAt), importanceScore: Math.min(100, Math.round(35 + (circlePriority[source.circleId] || 50) * 0.25 + Number(source.priority || 50) * 0.25 + recencyScore(snippet.publishedAt))), updatedAt: new Date() };
      const result = await db.collection('circle_updates').updateOne({ userId, externalUpdateId: mutable.externalUpdateId }, { $set: mutable, $setOnInsert: { id: uuidv4(), createdAt: new Date() } }, { upsert: true });
