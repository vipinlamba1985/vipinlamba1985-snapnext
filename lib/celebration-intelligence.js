...
const GENERIC_PEOPLE = new Set([
  'person', 'people', 'unknown', 'face', 'user', 'self', 'you', 'man', 'woman', 'boy', 'girl',
...
function countryLookup() {
  ...
  for (let first = 65; first <= 90; first += 1) {
    for (let second = 65; second <= 90; second += 1) {
      ...
      byName.set(label.toLowerCase(), code);
      labels.set(code, label);
    }
  }
  const aliases = {
    usa: 'US', 'united states of america': 'US', america: 'US', uk: 'GB', britain: 'GB', 'great britain': 'GB',
...
export function extractCountrySignals({ profiles = [], media = [] } = {}) {
  ...
  for (const item of media) {
    ...
    for (const candidate of candidates) addCountrySignal(signals, candidate, 'visited', `You have memories connected to ${countryLabel(countryCodeFor(candidate))}`);
  }
...
function personForMedia(item, peopleByCluster = {}) {
  for (const clusterId of item.peopleIntelligence?.clusterIds || []) {
    const label = validPersonName(peopleByCluster[String(clusterId)]);
  ...
  return values.map(validPersonName).find(Boolean) || null;
}

function stableSuggestionId({ type, personName, monthDay }) {
  return createHash('sha256').update(`${type}|${String(personName || '').toLowerCase()}|${monthDay}`).digest('hex').slice(0, 24);
}
...
export function buildMemoryCelebrationSuggestions({ media = [], profiles = [], events = [], peopleByCluster = {}, feedbackIds = [] } = {}) {
  ...
  for (const item of media) {
    ...
    if (!moment) continue;
    const personName = personForMedia(item, peopleByCluster);
    const monthDay = `${String(moment.date.getMonth() + 1).padStart(2, '0')}-${String(moment.date.getDate()).padStart(2, '0')}`;
    const key = `${type}|${String(personName || '').toLowerCase()}|${monthDay}`;
    if (!groups.has(key)) groups.set(key, { type, personName, monthDay, samples: [] });
  ...
  for (const group of groups.values()) {
    ...
    const profile = group.personName ? profileByName.get(group.personName.toLowerCase()) : null;
    ...
    const personPrefix = group.personName ? `${group.personName}'s ` : '';
