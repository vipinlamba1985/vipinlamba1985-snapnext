...
function previewResponse(path, options = {}) {
  ...
  if (path === '/media' || path.startsWith('/media?')) return { items: previewMedia, media: previewMedia, data: previewMedia, hasMore: false, nextCursor: null };
...
export function thumbnailSrc(id) {
  const t = getToken();
  if (t === PREVIEW_TOKEN) return '';
  return `/api/media/${encodeURIComponent(id)}/file?variant=thumbnail`;
}
