...
export async function apiFetch(path, options = {}) {
  ...
  if (token === PREVIEW_TOKEN) {
    // Allow entitlement-sensitive endpoints to pass through so server-side Developer Test Mode cookies apply.
    const passThrough = path.startsWith('/ai/') || path.startsWith('/agent-actions') || path.startsWith('/dev/') || path === '/auth/me' || path === '/storage/usage' || path === '/insights' || path === '/ai/status';
    if (!passThrough) {
