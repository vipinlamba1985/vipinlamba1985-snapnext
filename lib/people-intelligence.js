...
export function personThumbnailStyle(person = {}) {
  const override = person.thumbnailOverride;
  if (!override) return faceCropStyle(person.representativeFaceBox || {});
  const scale = Math.max(1, Math.min(8, Number(override.scale || 1)));
  const offsetX = Math.max(-100, Math.min(100, Number(override.offsetX || 0)));
  const offsetY = Math.max(-100, Math.min(100, Number(override.offsetY || 0)));
  return {
    objectPosition: '50% 50%',
...
export function cleanCluster(cluster = {}) {
  return {
    ...
    candidateMediaIds: Array.from(new Set(cluster.mediaIds || [])).slice(0, 12),
    thumbnailOverride: cluster.thumbnailOverride || null,
    relationship: String(cluster.relationship || ''),
    birthday: String(cluster.birthday || ''),
    phone: String(cluster.phone || ''),
    email: String(cluster.email || ''),
