...
export const FACE_MATCH_THRESHOLD = Number(process.env.PEOPLE_FACE_MATCH_THRESHOLD || 92);
...
export function peopleCollectionId(userId) {
  const hash = crypto.createHash('sha256').update(String(userId || '')).digest('hex').slice(0, 32);
  return `snapnext_${hash}`;
}
