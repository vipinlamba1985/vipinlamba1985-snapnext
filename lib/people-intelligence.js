...
import { normalizePeopleIdentityState } from '@/lib/people-identity';
...
export function cleanCluster(cluster = {}) {
  const historicalCount = Array.isArray(cluster.mediaIds) ? cluster.mediaIds.length : Number(cluster.memoryCount || 0);
  const liveCount = normalizedCount(cluster.liveMemoryCount);
  const livePhotos = normalizedCount(cluster.livePhotoCount);
  const liveVideos = normalizedCount(cluster.liveVideoCount);
  return {
    clusterId: cluster.clusterId,
    name: cluster.clusterId,
    displayName: clusterDisplayName(cluster),
    count: liveCount ?? historicalCount,
    photos: livePhotos ?? Number(cluster.photoCount || historicalCount),
    videos: liveVideos ?? Number(cluster.videoCount || 0),
    ...
    identityState: normalizePeopleIdentityState(cluster.identityState),
    isSelf: Boolean(cluster.isSelf),
  };
}
