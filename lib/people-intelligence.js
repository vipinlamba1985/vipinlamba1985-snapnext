...
export function isUsableFaceBox(box = {}) {
  ...
  return width >= 0.035 && height >= 0.035 && left >= 0 && top >= 0 && left + width <= 1.02 && top + height <= 1.02;
}

export function faceCropStyle(box = {}) {
  const width = Math.max(0.035, Number(box.Width || 0.3));
  const height = Math.max(0.035, Number(box.Height || 0.3));
  ...
  const faceSpan = Math.max(width, height);
  const scale = Math.max(1.75, Math.min(8, 0.62 / faceSpan));
  return {
    objectPosition: `${(centerX * 100).toFixed(2)}% ${(centerY * 100).toFixed(2)}%`,
    transform: `scale(${scale})`,
    transformOrigin: `${(centerX * 100).toFixed(2)}% ${(centerY * 100).toFixed(2)}%`,
