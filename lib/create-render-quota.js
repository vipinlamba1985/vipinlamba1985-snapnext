...
export async function settleCanonicalRenderQuota({ db, reservationId, artifactId = null, now = new Date() }) {
  ...
  let reservation = await db.collection('render_quota_reservations').findOne({ id: reservationId });
  if (!reservation) return { settled: false, reason: 'render_quota_reservation_missing' };
  if (reservation.status === 'settled') {
    return { settled: true, idempotent: true, usageId: reservation.usageId };
  }
  if (reservation.status !== 'reserved' && reservation.status !== 'settling') {
    return { settled: false, reason: 'render_quota_reservation_not_active' };
  }

  if (reservation.status === 'reserved') {
    const claimed = resultDocument(await db.collection('render_quota_reservations').findOneAndUpdate(
      { id: reservationId, status: 'reserved' },
      { $set: { status: 'settling', artifactId, settlementStartedAt: now } },
      { returnDocument: 'after' },
    ));
    if (claimed) reservation = claimed;
    else {
      reservation = await db.collection('render_quota_reservations').findOne({ id: reservationId });
      if (reservation?.status === 'settled') return { settled: true, idempotent: true, usageId: reservation.usageId };
      if (reservation?.status !== 'settling') return { settled: false, reason: 'render_quota_reservation_not_active' };
    }
  }

  const usage = await db.collection('render_quota_usage').updateOne(
    {
      _id: reservation.usageId,
      settledReservationIds: { $ne: reservationId },
      reserved: { $gt: 0 },
    },
    {
      $inc: { reserved: -1, used: 1 },
      $addToSet: { settledReservationIds: reservationId },
      $set: { updatedAt: now },
    },
  );

  if (usage.matchedCount !== 1) {
    const usageRow = await db.collection('render_quota_usage').findOne({ _id: reservation.usageId });
    if (!usageRow?.settledReservationIds?.includes(reservationId)) {
      const error = new Error('Render quota usage could not be settled atomically.');
      error.code = 'render_quota_usage_settlement_failed';
      throw error;
    }
  }

  await db.collection('render_quota_reservations').updateOne(
    { id: reservationId, status: 'settling' },
    { $set: { status: 'settled', artifactId, settledAt: now } },
  );
  return { settled: true, usageId: reservation.usageId };
}
