...
export async function runFavoriteAction(user, favoriteId, requestedAction) {
  ...
  if (action === 'remove') {
    const [userAlbums, otherAlbums] = await Promise.all([
      db.collection('shared_albums').find({ ownerUserId: user.id }).project({ _id: 0, id: 1 }).toArray(),
      db.collection('shared_albums').find({ ownerUserId: otherId }).project({ _id: 0, id: 1 }).toArray(),
    ]);
