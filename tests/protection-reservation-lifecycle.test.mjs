...
test('the registry prevents a stale generation replacing or cancelling the current queue', () => {
  ...
  assert.deepEqual(registry.preparedIds(first), ['old']);
  assert.deepEqual(registry.preparedIds(second), ['new']);
  ...
  assert.deepEqual(registry.queueOwnedIds(), ['new'], 'review cleanup must not release queue-owned work');
});
...
test('expired reservations are reclaimed before a new quota decision', async () => {
  ...
  assert.ok(reservation, 'expired quota must not block the new reservation');
...
test('active reservations still reduce available quota', async () => {
  ...
  assert.equal(db.rows('upload_quota_ledgers')[0].reservedBytes, 80);
});
...
test('Commit A0 preserves the visible upload state machine while adding cleanup guards', async () => {
  ...
  for (const state of ['welcome', 'report', 'priority', 'review', 'protecting', 'results']) {
    assert.match(discovery + hook, new RegExp(`['\"]${state}['\"]`));
  }
  assert.match(discovery, /Build My Protection Plan/);
  assert.match(discovery, /Protect These Memories/);
