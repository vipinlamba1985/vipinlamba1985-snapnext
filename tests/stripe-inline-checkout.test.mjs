...
test('inline Stripe pricing converts authoritative monthly and yearly amounts to cents', () => {
  const monthly = inlineStripeLineItem(PLANS.starter, 'monthly');
  const yearly = inlineStripeLineItem(PLANS.pro, 'yearly');
  assert.equal(monthly.price_data.unit_amount, 99);
  ...
  assert.equal(monthly.price_data.product_data.metadata.snapnextPlanId, 'starter');
});
