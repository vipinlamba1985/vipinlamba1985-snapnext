  ...
  try {
    ...
    if (route === '/memories/timeline' && method === 'GET') {
      ...
      for (const m of all) {
        ...
        // Relationship Timeline — keyword derived purely from AI-detected
        // roles/tags, no user-specific personal names. Do NOT reintroduce
        // any hardcoded personal names here — that would leak owner PII
        // into every user's timeline classification.
      ...
      // ---------------------------------------------------------------
      // Cycle recaps — TRUTHFUL, DERIVED FROM REAL COUNTS ONLY.
      // ---------------------------------------------------------------
      // Prior versions returned pre-written promotional narratives here
      // (e.g. "Year 2026 was defined by travel landmarks, heartwarming
      // family gatherings ..."). Those strings were fabricated and are
      // a launch blocker under SnapNext's zero-fabrication rule.
      //
      // Recap text is now built strictly from `db.collection('media')`
      // counts and the actual current month/year label. If a client
      // needs a longer narrative it should call `/ai/memory-summary`
      // with real facts, not consume a canned string from this endpoint.
      const now = new Date();
      const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
      const yearStart = new Date(now.getFullYear(), 0, 1);
      const monthCount = all.filter(m => new Date(m.createdAt) >= monthStart).length;
      const yearCount = all.filter(m => new Date(m.createdAt) >= yearStart).length;
      const monthLabel = now.toLocaleString('en-US', { month: 'long', year: 'numeric' });
      const yearLabel = String(now.getFullYear());

      const monthlyRecap = monthCount > 0
        ? `${monthCount} ${monthCount === 1 ? 'memory' : 'memories'} captured in ${monthLabel}.`
        : '';

      const yearlyRecap = yearCount > 0
        ? `${yearCount} ${yearCount === 1 ? 'memory' : 'memories'} captured so far in ${yearLabel}.`
        : '';
    ...
    if (route === '/favorites/ai' && method === 'GET') {
      ...
      // Truthful suggestions and highlights built from real face-tag counts.
      // Prior versions fabricated emotional narratives ("You share the most
      // emotional, joyful moments with X"). Those claims aren't supported by
      // the underlying data and violate SnapNext's zero-fabrication rule.
      const topPerson = favoritePeople[0];
      const suggestions = topPerson
        ? [
            `"${topPerson.name}" appears in ${topPerson.count} ${topPerson.count === 1 ? 'memory' : 'memories'} in your library.`,
            `Create a shared album focused on ${topPerson.name}.`,
          ]
        : ['Add photos with recognisable faces so SnapNext can group them into people-based timelines.'];

      const totalWithFaces = Object.values(faceCounts).reduce((a, b) => a + b, 0);

      return json({
        favoritePeople,
        suggestions,
        relationshipHighlights: favoritePeople.length > 0
          ? `${favoritePeople.length} recognised ${favoritePeople.length === 1 ? 'person' : 'people'} across ${totalWithFaces} face signals in your memories.`
          : 'No recognised people yet. Face grouping will appear here as memories are added.',
      });
    }
    ...
    if (route === '/ai/audio-transcribe' && method === 'POST') {
      ...
      // Transcription must be truthful — no default narrative strings.
      // If the transcription provider is unavailable, return an explicit
      // empty transcript with a `providerStatus` marker so the client can
      // surface an honest "transcript unavailable" state.
      let text = '';
      let providerStatus = 'ok';
      try {
        const buf = await storage.read({ provider: doc.provider || 'local', storageKey: doc.storageKey });
        const transcribed = await transcribeAudio({ buffer: buf, mimeType: doc.mime });
        // transcribeAudio can return either a plain string (legacy) or an
        // object with { providerStatus, text }. Handle both defensively so
        // we never leak the old fabricated placeholder string to the UI.
        if (typeof transcribed === 'string') {
          text = transcribed;
        } else if (transcribed && typeof transcribed === 'object') {
          text = transcribed.text || '';
          providerStatus = transcribed.providerStatus || 'ok';
        }
        if (text) {
      ...
      return json({
        transcript: text,
        providerStatus,
        meta: { creditsUsed: q.credits, plan: q.plan, requestId: q.requestId },
      });
    }
