...
export async function GET(request, routeContext) {
  ...
  const { user, db, action, familySafety } = ctx; const [resource, id] = action;
...
export async function POST(request, routeContext) {
  const ctx = await context(request, routeContext); if (ctx.error) return ctx.error;
  const { user, db, action, familySafety } = ctx; const [resource, id, subresource] = action; const body = await request.json().catch(() => ({}));
  if (minorBlocked(familySafety)) return json({ error: 'Family Safety approval is required before this minor account can use chats or communities.' }, 403);
  ...
  if (resource === 'threads' && !id) {
    ...
    if (familySafety.isMinor && type === 'community') return json({ error: 'A guardian must create or approve communities for a minor account.' }, 403);
    if (type === 'direct') {
  ...
  if (resource === 'threads' && id && subresource === 'respond') {
    const thread = await memberThread(db, user.id, id); if (!thread || thread.type !== 'direct' || thread.status !== 'pending') return json({ error: 'Chat request not found or still awaiting guardian approval.' }, 404); if (thread.requestRecipientId !== user.id) return json({ error: 'Only the invited person can respond.' }, 403);
  ...
  if (resource === 'threads' && id && subresource === 'messages') {
    const thread = await memberThread(db, user.id, id); if (!thread) return json({ error: 'Conversation not found.' }, 404);
    if (!canPost(thread, user.id)) { if (thread.type === 'direct' && thread.status !== 'active') return json({ error: 'This private chat must complete guardian approval and recipient acceptance before messages or memories can be shared.' }, 403); return json({ error: 'You can view this community, but the owner has not given you posting permission.' }, 403); }
    const content = text(body.content, 2000); const stickerId = text(body.stickerId, 120); const memoryIds = [...new Set(Array.isArray(body.memoryIds) ? body.memoryIds.map(String) : [])].slice(0, 10); if (!content && !memoryIds.length && !stickerId) return json({ error: 'Write a message, share a memory, or choose a sticker first.' }, 400);
    ...
    let sticker = null; if (stickerId) { const found = await db.collection('media').findOne({ id: stickerId, userId: user.id, trashed: { $ne: true }, kind: 'photo' }); if (!found) return json({ error: 'This photo sticker is no longer available.' }, 400); sticker = publicMemory(found); }
    const message = { id: uuidv4(), threadId: id, senderId: user.id, sender: publicUser(user), type: sticker ? 'sticker' : 'message', content, memories, memoryIds: memories.map(memory => memory.id), sticker, stickerId: sticker?.id || null, createdAt: new Date(), editedAt: null };
    await db.collection('chat_messages').insertOne(message);
    const preview = sticker ? 'Sent a memory sticker' : content || (memories.length === 1 ? 'Shared a memory' : `Shared ${memories.length} memories`);
