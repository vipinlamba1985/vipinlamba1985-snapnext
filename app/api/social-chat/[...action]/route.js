...
async function context(request, routeContext) { const user = await getUserFromRequest(request); if (!user) return { error: json({ error: 'Please sign in again.' }, 401) }; const db = await getDb(); const action = (await routeContext.params).action || []; return { user, db, action }; }
...
export async function GET(request, routeContext) {
  ...
  const { user, db, action } = ctx; const [resource, id] = action;
  if (resource === 'threads' && !id) {
    ...
    return json({ currentUserId: user.id, threads: threads.map(({ _id, ...thread }) => ({ ...thread, currentUserPermission: permissionFor(thread, user.id), canPost: canPost(thread, user.id), isRequestRecipient: thread.requestRecipientId === user.id })) });
  }
  if (resource === 'threads' && id) {
...
export async function POST(request, routeContext) {
  const ctx = await context(request, routeContext); if (ctx.error) return ctx.error;
  const { user, db, action } = ctx; const [resource, id, subresource] = action; const body = await request.json().catch(() => ({}));
  if (resource === 'threads' && !id) {
    const type = body.type === 'community' ? 'community' : 'direct'; const email = text(body.email, 320).toLowerCase(); const name = text(body.name, 80); let members = [publicUser(user)]; let target = null;
    if (type === 'direct') {
    ...
    const thread = { id: uuidv4(), type, name: type === 'community' ? name : '', ownerId: user.id, members, memberIds: members.map(member => member.id), memberPermissions, memberKey: type === 'direct' ? members.map(member => member.id).sort().join(':') : undefined, status: type === 'direct' ? 'pending' : 'active', requestSenderId: type === 'direct' ? user.id : undefined, requestRecipientId: type === 'direct' ? target.id : undefined, private: true, purpose: type === 'community' ? 'memory_discussion' : 'private_chat', createdAt: now, updatedAt: now, lastMessageAt: now, lastMessage: type === 'direct' ? 'Chat request pending' : '', archivedFor: [] };
    await db.collection('chat_threads').insertOne(thread); return json({ thread: { ...thread, _id: undefined } }, 201);
  }
  if (resource === 'threads' && id && subresource === 'respond') {
    const thread = await memberThread(db, user.id, id); if (!thread || thread.type !== 'direct' || thread.status !== 'pending') return json({ error: 'Chat request not found.' }, 404); if (thread.requestRecipientId !== user.id) return json({ error: 'Only the invited person can respond.' }, 403);
    const decision = body.decision === 'accept' ? 'accept' : body.decision === 'decline' ? 'decline' : ''; if (!decision) return json({ error: 'Choose accept or decline.' }, 400); const now = new Date();
    if (decision === 'decline') { await db.collection('chat_threads').updateOne({ id }, { $set: { status: 'declined', lastMessage: 'Chat request declined', updatedAt: now, lastMessageAt: now } }); return json({ ok: true, status: 'declined' }); }
    await db.collection('chat_threads').updateOne({ id }, { $set: { status: 'active', acceptedAt: now, acceptedBy: user.id, lastMessage: 'Chat request accepted', updatedAt: now, lastMessageAt: now } }); return json({ ok: true, status: 'active' });
  }
  if (resource === 'threads' && id && subresource === 'messages') {
    const thread = await memberThread(db, user.id, id); if (!thread) return json({ error: 'Conversation not found.' }, 404);
    if (!canPost(thread, user.id)) { if (thread.type === 'direct' && thread.status !== 'active') return json({ error: 'This private chat must be accepted before messages or memories can be shared.' }, 403); return json({ error: 'You can view this community, but the owner has not given you posting permission.' }, 403); }
    const content = text(body.content, 2000); const memoryIds = [...new Set(Array.isArray(body.memoryIds) ? body.memoryIds.map(String) : [])].slice(0, 10); if (!content && !memoryIds.length) return json({ error: 'Write a message or share a memory first.' }, 400);
  ...
  if (resource === 'threads' && id && subresource === 'permissions') {
    const thread = await memberThread(db, user.id, id); if (!thread || thread.type !== 'community') return json({ error: 'Community not found.' }, 404); if (thread.ownerId !== user.id) return json({ error: 'Only the community owner can change posting permissions.' }, 403);
    const memberId = text(body.memberId, 120); if (!memberId || memberId === thread.ownerId || !thread.memberIds.includes(memberId)) return json({ error: 'Choose a valid community member.' }, 400); const permission = body.permission === 'post' ? 'post' : 'view'; await db.collection('chat_threads').updateOne({ id }, { $set: { [`memberPermissions.${memberId}`]: permission, updatedAt: new Date() } }); return json({ ok: true, memberId, permission });
  }
  return json({ error: 'Not found.' }, 404);
}