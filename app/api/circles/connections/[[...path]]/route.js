...
import { buildSourceDocument, cleanMongo } from '@/lib/circles';
import {
  ...
  try {
    ...
    if (route === '/youtube/callback' && method === 'GET') {
      ...
      await db.collection('circle_connections').updateOne(
        ...
        { $set: { ...connection, createdAt: undefined }, $setOnInsert: { id: connection.id, createdAt: connection.createdAt } },
        { upsert: true },
      );
    ...
    if (route === '/candidates/accept' && method === 'POST') {
      ...
      for (const candidate of candidates) {
        const source = buildSourceDocument(user.id, circleId, {
          platform: candidate.platform,
          profileUrl: candidate.profileUrl,
          input: candidate.profileUrl,
