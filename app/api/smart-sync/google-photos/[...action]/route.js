...
async function authContext(request, routeContext) {
...
async function connectedContext(db, userId) {
  const [connection, profile] = await Promise.all([
    db.collection('cloud_connections').findOne({ userId, provider: 'google_photos' }),
    db.collection('smart_sync_profiles').findOne({ userId, providerId: 'google_photos' }),
  ]);
  if (!connection) throw new Error('Connect Google Photos first.');
  if (!profile?.enabled || !profile?.approvedAt) throw new Error('Review and approve the Google Photos Smart Sync plan first.');
...
export async function POST(request, routeContext) {
  ...
  try {
    const { connection, profile } = await connectedContext(db, user.id);
    ...
    const session = await createGooglePhotosPickerSession(token, body.maxItemCount || 500);
    const now = new Date();
    await db.collection('smart_sync_picker_sessions').updateOne(
      { userId: user.id, provider: 'google_photos', sessionId: session.id },
      {
        $set: {
          ...
          status: 'selecting',
          syncMode: normalizeSmartSyncMode(profile.syncMode),
          pickerUri: session.pickerUri,
          expiresAt: session.expireTime ? new Date(session.expireTime) : new Date(Date.now() + 60 * 60 * 1000),
          updatedAt: now,
        },
    ...
    return json({
      sessionId: session.id,
      pickerUri: session.pickerUri ? `${session.pickerUri.replace(/\/$/, '')}/autoclose` : null,
      pollInterval: session.pollingConfig?.pollInterval || '3s',
...
export async function GET(request, routeContext) {
  ...
  if (localSession.jobId) {
    ...
    return json({ ready: true, job: job ? publicSmartSyncJob(job) : null });
  }
  ...
  try {
    ...
    if (!remoteSession.mediaItemsSet) {
      return json({
        ready: false,
        pollInterval: remoteSession.pollingConfig?.pollInterval || '3s',
        timeoutIn: remoteSession.pollingConfig?.timeoutIn || null,
      });
    }
    ...
    const syncMode = normalizeSmartSyncMode(profile.syncMode);
    const sourceFileIds = syncMode === 'index_only' ? [] : inventory.importable;
    const automaticallySkipped = inventory.safeExisting + inventory.unsupported;
    const indexedOnlyItems = syncMode === 'index_only' ? inventory.importable.length : 0;
    const metrics = normalizeSyncMetrics({
      discoveredItems: rawItems.length,
      indexedItems: inventory.importable.length,
      metadataUpserts: rawItems.length,
      providerApiCalls: 1 + Math.max(1, Math.ceil(rawItems.length / 100)),
      unsupportedItems: inventory.unsupported,
    });

    const activeKey = `${user.id}:google_photos`;
    const existing = await db.collection('smart_sync_jobs').findOne({ activeKey });
    if (existing) return json({ ready: true, job: publicSmartSyncJob(existing), existing: true });
    const job = {
      id: uuidv4(),
      ...createSmartSyncJob({
        userId: user.id,
        ...
        estimate: { items: rawItems.length },
      }),
      pickerSessionId: sessionId,
      estimatedItems: rawItems.length,
      processedItems: indexedOnlyItems + automaticallySkipped,
      indexedItems: inventory.importable.length,
      skippedItems: automaticallySkipped,
      metrics,
    };
    ...
    await db.collection('smart_sync_picker_sessions').updateOne(
      ...
      { $set: { status: 'ready', jobId: job.id, itemCount: rawItems.length, updatedAt: new Date() } },
    );
    return json({ ready: true, job: publicSmartSyncJob(job), itemCount: rawItems.length }, 201);
  } catch (error) {
    await db.collection('smart_sync_picker_sessions').updateOne(
      { _id: localSession._id },
      { $set: { status: 'failed', lastError: String(error.message || 'Google Photos selection failed').slice(0, 500), updatedAt: new Date() } },
    );
...
export async function DELETE(request, routeContext) {
  ...
  try {
    const connection = await db.collection('cloud_connections').findOne({ userId: user.id, provider: 'google_photos' });
    if (connection) {
      const token = await freshProviderAccessToken(db, connection);
      await deleteGooglePhotosPickerSession(token, sessionId);
    }
  } catch {}
