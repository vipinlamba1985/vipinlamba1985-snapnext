import { getUserFromRequest } from '@/lib/auth';
import { listSharedMemories, shareMemory } from '@/lib/sharing/api-service';
import { sharingError, sharingJson } from '@/lib/sharing/route-response';

export const dynamic = 'force-dynamic';
export const runtime = 'nodejs';

export async function GET(request) {
  try {
    const user = await getUserFromRequest(request);
    if (!user) return sharingJson({ error: 'Unauthorized', code: 'auth_unauthorized' }, 401);
    return sharingJson(await listSharedMemories(user));
  } catch (error) {
    return sharingError(error);
  }
}

export async function POST(request) {
  try {
    const user = await getUserFromRequest(request);
    if (!user) return sharingJson({ error: 'Unauthorized', code: 'auth_unauthorized' }, 401);
    const body = await request.json().catch(() => ({}));
    return sharingJson(await shareMemory(user, body));
  } catch (error) {
    return sharingError(error);
  }
}
