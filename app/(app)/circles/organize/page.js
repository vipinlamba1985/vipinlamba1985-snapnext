...
import { BellRing, Bookmark, CalendarClock, Check, ChevronRight, Clock3, ExternalLink, Link2, Loader2, Plus, ShieldCheck, Sparkles, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-3 sm:grid-cols-4">
        <Stat label="Needs attention" value={data.counts?.important || 0} />
        <Stat label="In your digest" value={data.counts?.digest || 0} />
        <Stat label="Organized quietly" value={data.counts?.organizedSilently || 0} />
        <Stat label="Total organized" value={totalVisible} />
      </section>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {Object.entries(MODE_COPY).map(([mode, description]) => {
            const active = data.preferences?.mode === mode;
            return <button key={mode} onClick={() => setMode(mode)} disabled={busy === `mode-${mode}`} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-pink-400/50 bg-pink-500/15' : 'border-white/10 bg-black/10 hover:bg-white/5'}`}><div className="flex items-center justify-between"><span className="font-bold capitalize">{mode}</span>{active && <Check className="h-4 w-4 text-pink-300" />}</div><p className="mt-2 text-xs leading-5 text-white/55">{description}</p></button>;
          })}
        </div>
      </section>

      <section className="grid gap-4 xl:grid-cols-3">
        {BUCKETS.map((bucket) => <Bucket key={bucket.id} bucket={bucket} items={data.sections?.[bucket.id] || []} busy={busy} onMove={move} onRemove={remove} />)}
      </section>

      <section className="rounded-3xl border border-purple-400/20 bg-gradient-to-br from-purple-500/15 to-pink-500/5 p-5">
        <div className="flex items-start gap-3"><Sparkles className="mt-0.5 h-5 w-5 text-purple-200" /><div><h2 className="font-bold">Proof of the calm model</h2><p className="mt-1 text-sm leading-6 text-white/60">This build performs deterministic filtering first. AI is reserved for future batch digests and questions, preventing one costly model call for every saved item. Your badge can represent important unread items instead of raw activity.</p></div></div>
      </section>
