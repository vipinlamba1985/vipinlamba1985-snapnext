...
export default function DiscoveryFlow() {
  ...
  async function chooseFiles(files) {
    ...
    const applied = await flow.replaceItems(items);
    if (applied) flow.setStage('report');
  }
  ...
  async function startProtection() {
    ...
    try {
      ...
      handoff = flow.handoffPreparedReservations(decisions);
      ...
      flow.setProtecting(false);
      if (error?.code !== 'protection_preparation_cancelled') flow.setStage('review');
    } finally {
  ...
  if (flow.stage === 'welcome') return (
    <div className="mx-auto max-w-5xl pb-36 md:pb-12"><section className="rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-pink-500/15 via-purple-600/10 to-cyan-500/10 p-6 text-center md:p-12">
      ...
      <input ref={inputRef} type="file" multiple accept="image/*,video/*" onChange={(event) => { void chooseFiles(Array.from(event.target.files || [])); event.target.value = ''; }} className="hidden" />
  ...
  if (flow.stage === 'report') {
    ...
    return <div className="mx-auto max-w-5xl space-y-6 pb-36 md:pb-12"><section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 md:p-8">
      ...
      <div className="mt-6 flex flex-wrap gap-3"><button onClick={() => flow.setStage('priority')} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-black text-black">Choose What to Protect <ArrowRight className="h-4 w-4" /></button><button onClick={() => { void flow.resetFlow(); }} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/65">Start Again</button></div>
    </section></div>;
  }
  ...
  if (flow.stage === 'review') return (
    <div className="mx-auto max-w-4xl space-y-6 pb-36 md:pb-12"><section className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-emerald-400/10 via-white/[0.03] to-purple-500/10 p-6 md:p-8">
      ...
      <div className="mt-7 flex flex-wrap gap-3"><button onClick={startProtection} disabled={!flow.plan.selected.length} className="rounded-full bg-gradient-to-r from-pink-500 to-purple-600 px-7 py-3 text-sm font-black text-white disabled:opacity-40">Protect These Memories</button><button onClick={() => flow.setStage('priority')} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/65">Adjust My Plan</button><button onClick={() => { void flow.resetFlow(); }} className="rounded-full px-4 py-3 text-sm text-white/40">Cancel</button></div>
    </section></div>
  );
