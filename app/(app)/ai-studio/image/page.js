...
export default function CreateImagePage() {
  ...
  useEffect(() => {
    Promise.all([apiFetch('/ai-create-image'), apiFetch('/media?filter=photo')])
      ...
      .catch(() => toast.error('We could not open Create Image. Please try again in a moment.'));
  }, []);

  const active = templates.find((item) => item.id === templateId);

  async function createImage() {
    if (!prompt.trim()) return toast.error('Tell us what you would like to create.');
    setBusy(true);
    setResult(null);
    try {
      ...
      toast.error(error.message || "We couldn't finish this one. Your Credits were not used.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="space-y-6 pb-24">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/60">Turn an idea into a beautiful image, or give one of your photos a fresh new look.</p>
        </div>
        <div className={`rounded-full px-3 py-1.5 text-xs font-bold ${providerReady ? 'bg-emerald-500/15 text-emerald-200' : 'bg-amber-500/15 text-amber-100'}`}>{providerReady ? 'Ready to create' : 'Coming soon'}</div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="space-y-5 rounded-[2rem] border border-white/10 bg-white/[0.035] p-5">
          <div>
            <label className="text-xs font-bold text-white/55">What would you like to create?</label>
            <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} maxLength={1200} placeholder="Example: A warm sunset over Goa in a travel-poster style" className="mt-2 min-h-32 w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none focus:border-cyan-300/60"/>
          </div>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5">
          ...
          <div className="mt-4 grid min-h-80 place-items-center overflow-hidden rounded-3xl border border-dashed border-white/10 bg-black/20">
            {result?.outputUrl ? <img src={result.outputUrl} alt="Your creation" className="h-full max-h-[32rem] w-full object-contain"/> : mediaId ? <img src={mediaSrc(mediaId)} alt="Selected photo" className="h-full max-h-80 w-full object-contain opacity-65"/> : <div className="px-6 text-center text-sm leading-6 text-white/35">Pick a style, describe your idea, and add a photo only if you want to.</div>}
          </div>
          {!providerReady && <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-500/10 p-3 text-xs leading-5 text-amber-100/80">Image creation is coming soon. You can explore the styles now, and we will let you know when creating is available.</div>}
        </section>
      </div>
    </div>
  );
